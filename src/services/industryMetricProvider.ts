...
// Offline artifact validated byte-for-byte by data:validate:industry. No network or mock fallback.
export const roboticsMetric = retained as IndustryMetricDataset;
export function industryMetric(industryId: string): IndustryMetricDataset | null {
  return industryId === roboticsMetric.definition.industryId ? roboticsMetric : null;
}
...
export function industryHistory(owner: IndustryMetricDataset, basis: IndustryMetricBasis) {
  ...
  const delta = basis === 'monthly' && latest?.value !== null && latest?.value !== undefined && previous?.value !== null && previous?.value !== undefined
...
export function industryChartAudit(owner: IndustryMetricDataset, basis: IndustryMetricBasis): ChartAuditView {
  ...
  return { title: `${d.canonicalName} · ${basis}`, scope: `${d.industryId} · ${d.geography} · ${d.scope}`, quality: view.states,
    rows: [row('Metric owner', d.id), row('Industry identity（非 Entity Registry 映射）', d.industryId), row('Entity Registry', owner.policy.entityResolution),
      ...
      row('F3', 'Frozen service adapter NOT_IMPLEMENTED；未提升覆盖'), row('计算', '当月绝对量直接读取；差额=同年相邻月留存值之差，非官方环比增速；累计值不作环比；缺失不连接'),

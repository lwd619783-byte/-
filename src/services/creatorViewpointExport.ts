...
import { buildCreatorCurrentViews, buildViewpointReviews, buildViewpointTimeline, validateCreatorViewpointData, visibleViewpointObservations } from './creatorViewpoint';
...
export function exportCreatorViewpointExcel(data: CreatorViewpointData, asOf?: string): Uint8Array {
  ...
  const sheetList: Sheet[] = [
    ...
    { name: 'Timeline', headers: ['Observation ID', 'Creator ID', 'Creator', 'Topic ID', 'Topic', 'Recorded at', 'Summary', 'Reasoning', 'Stance', 'Conditional', 'Horizon', 'Trigger', 'Confirmation', 'Invalidation', 'Source ID', 'Event relations', 'Supersedes', 'Revision reason', 'Important', 'Review decisions', 'Transition kind', 'Previous observation', 'Transition at', 'Semantic class'], rows: observations.map(row => {
      ...
      return [row.id, row.creatorId, creatorName(row.creatorId), row.topicId, topicName(row.topicId), row.recordedAt, row.summary, row.reasoning, row.stance, row.conditional, row.horizon, row.trigger, row.confirmation, row.invalidation, row.sourceId, JSON.stringify(row.eventLinks), row.supersedesId, row.revisionReason, row.important, decisions.length ? JSON.stringify(decisions) : 'draft', transition ? transition.previous ? 'state_transition' : 'initialization' : 'no_transition', transition?.previous?.id, transition?.recordedAt, data.semanticClass];
    }) },
    ...
    { name: 'Reviews', headers: ['Row kind', 'Review ID', 'Observation ID', 'Original viewpoint', 'Trigger', 'Confirmation', 'Invalidation', 'Offset days', 'Calendar basis', 'Due at', 'Recorded at', 'Status', 'Trigger occurred', 'Invalidation occurred', 'Actual outcome', 'Evidence', 'Evidence URL', 'Supersedes'], rows: [] },
  ];
  ...
  for (const review of data.reviews.filter(row => visible(row) && observationIds.has(row.observationId))) {
    ...
    reviewSheet.rows.push(['history', review.id, review.observationId, observation?.summary, observation?.trigger, observation?.confirmation, observation?.invalidation, review.offsetDays, 'calendar_days', null, review.recordedAt, review.status, review.triggerOccurred, review.invalidationOccurred, review.actualOutcome, review.evidence, review.evidenceUrl, review.supersedesId]);
  }

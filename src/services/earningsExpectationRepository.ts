...
export interface CsvImportOptions {
  ...
  validStocks: Array<{ id: string; code: string; market: Market }>;
  now?: Date;
}
...
export function migrateEarningsExpectationEnvelope(value: unknown): EarningsExpectationStoreEnvelope {
  ...
  if (!Array.isArray(cloned.snapshots)) cloned.snapshots = [];
...
export class EarningsExpectationRepository {
  ...
  previewJson(raw: string | unknown, current: EarningsExpectationStoreEnvelope): EarningsExpectationImportPreview {
    ...
    return buildImportPreview(Array.isArray(parsed.snapshots) ? parsed.snapshots : [], current, EARNINGS_EXPECTATION_SCHEMA_VERSION);
  }
  ...
  previewCsv(raw: string, current: EarningsExpectationStoreEnvelope, options: CsvImportOptions): EarningsExpectationImportPreview {
    ...
    const preview = buildImportPreview(parsed.snapshots, current, EARNINGS_EXPECTATION_SCHEMA_VERSION);
...
export function validateEarningsExpectationSnapshot(snapshot: EarningsExpectationSnapshot): string[] {
  const errors: string[] = [];
  ...
  if (!validDate(snapshot.asOfDate)) errors.push("预期形成日期无效。");
  if (!validDate(snapshot.createdAt)) errors.push("录入时间无效。");
  if (snapshot.sourcePublishedAt !== null && !validDate(snapshot.sourcePublishedAt)) errors.push("来源发布日期无效。");
  if (snapshot.sourcePublishedAt && validDate(snapshot.createdAt) && Date.parse(snapshot.sourcePublishedAt) > Date.parse(snapshot.createdAt)) errors.push("来源发布日期不得晚于录入时间。");
  if (validDate(snapshot.asOfDate) && validDate(snapshot.createdAt) && Date.parse(snapshot.asOfDate) > Date.parse(snapshot.createdAt)) errors.push("预期形成日期不得晚于录入时间。");
  ...
  if (snapshot.sourceCategory === "institution_consensus" && (!snapshot.sourceName.trim() || !snapshot.sourceTitle.trim() || !safeSourceUrl(snapshot.sourceUrl))) errors.push("机构一致预期必须提供可核验的来源主体、标题和链接。");
  if (snapshot.sourceCategory === "institution_single" && !snapshot.sourceName.trim()) errors.push("单家机构预测必须填写机构名称。");
  if (snapshot.sourceCategory !== "user_estimate" && !snapshot.sourceTitle.trim()) errors.push("非用户预测必须填写来源标题。");
  if (snapshot.sourceCategory === "company_guidance" && !snapshot.sourceUrl && snapshot.sourceVerificationStatus === "verified") errors.push("公司指引缺少来源时不能标记为已核验。");
  if (snapshot.ingestionMethod === "provider") errors.push("本轮未实现自动 Provider，不能写入 provider 记录。");
...
export function parseEarningsExpectationCsv(raw: string, options: CsvImportOptions) {
  ...
  rows.slice(1).forEach((cells, offset) => {
    ...
    const record: Record<string, string> = Object.fromEntries([
      ...
      "sourcePublishedAt", "asOfDate", "analystCount", "institutionCount", "sourceVerificationStatus", "notes",
    ...
    if (!stock || !periodScope || !metric || !sourceCategory || !isReportPeriod(normalizeDate(record.reportPeriod))) return;
    ...
    const sourceVerificationStatus = verificationValue(record.sourceVerificationStatus) ?? (sourceCategory === "user_estimate" ? "verified" : "pending");
    const snapshot: EarningsExpectationSnapshot = {
      ...
      unit,
      accountingBasis: accountingBasisValue(record.accountingBasis) ?? "unknown",
      sourceCategory,
      sourceName: sourceCategory === "user_estimate" ? (record.sourceName || "用户个人预测") : record.sourceName,
      sourceTitle: record.sourceTitle,
      sourceUrl: record.sourceUrl || null,
      sourcePublishedAt: record.sourcePublishedAt ? normalizeDate(record.sourcePublishedAt) : null,
    ...
    const rowIssues = validateEarningsExpectationSnapshot(snapshot);
...
export function earningsExpectationCsvTemplate() {
  return "stockId,reportPeriod,periodScope,metric,estimateShape,value,lowerBound,upperBound,currency,unit,accountingBasis,sourceCategory,sourceName,sourceTitle,sourceUrl,sourcePublishedAt,asOfDate,analystCount,institutionCount,sourceVerificationStatus,notes\n";
}
...
export function exportEarningsExpectationCsv(snapshots: EarningsExpectationSnapshot[]) {
  ...
  const rows = snapshots.map((snapshot) => [snapshot.stockId, snapshot.reportPeriod, snapshot.periodScope, snapshot.metric, snapshot.estimateShape, snapshot.value, snapshot.lowerBound, snapshot.upperBound, snapshot.currency, snapshot.unit, snapshot.accountingBasis, snapshot.sourceCategory, snapshot.sourceName, snapshot.sourceTitle, snapshot.sourceUrl, snapshot.sourcePublishedAt, snapshot.asOfDate, snapshot.analystCount, snapshot.institutionCount, snapshot.sourceVerificationStatus, snapshot.notes].map(csvCell).join(","));
  return `\uFEFF${headers}\n${rows.join("\n")}`;
}

function buildImportPreview(values: unknown[], current: EarningsExpectationStoreEnvelope, schemaVersion: number): EarningsExpectationImportPreview {
  ...
  values.forEach((value, index) => {
    const snapshot = cloneJson(value) as EarningsExpectationSnapshot;
    const errors = validateEarningsExpectationSnapshot(snapshot);
    if (errors.length) errors.forEach((message) => issues.push({ row: index + 1, code: "invalid_snapshot", message }));
    else snapshots.push(snapshot);
  });
  ...
  return { ok: issues.length === 0 && conflictCount === 0, schemaVersion, totalCount: values.length, validCount: snapshots.length, addCount, duplicateCount, conflictCount, invalidCount: issues.filter((issue) => issue.code === "invalid_snapshot").length, issues, snapshots: unique };
}
...
function invalidPreview(message: string, code: string): EarningsExpectationImportPreview {
  return { ok: false, schemaVersion: null, totalCount: 0, validCount: 0, addCount: 0, duplicateCount: 0, conflictCount: 0, invalidCount: 1, issues: [{ row: 0, code, message }], snapshots: [] };
}
...
function estimateShapeValue(value: string, point: string, lower: string, upper: string): EarningsExpectationSnapshot["estimateShape"] { const key = value.trim().toLowerCase(); if (["range", "区间"].includes(key)) return "range"; if (["point", "点预测"].includes(key)) return "point"; return lower || upper ? "range" : point ? "point" : "point"; }
...
function isReportPeriod(value: string) { if (!validDate(value)) return false; return ["03-31", "06-30", "09-30", "12-31"].includes(value.slice(5)); }
function validDate(value: string) { if (!value || Number.isNaN(Date.parse(value))) return false; const dateOnly = value.slice(0, 10); const parsed = new Date(`${dateOnly}T00:00:00Z`); return !Number.isNaN(parsed.getTime()) && parsed.toISOString().slice(0, 10) === dateOnly; }
function periodScopeMatchesReportPeriod(period: string, scope: EarningsExpectationSnapshot["periodScope"]) { const suffix = period.slice(5); if (scope === "half_year") return suffix === "06-30"; if (scope === "first_three_quarters") return suffix === "09-30"; if (scope === "full_year") return suffix === "12-31"; if (scope === "year_to_date") return ["03-31", "06-30", "09-30", "12-31"].includes(suffix); return true; }
...
function safeSourceUrl(value: string | null) { if (!value) return false; try { const parsed = new URL(value); return parsed.protocol === "https:" || parsed.protocol === "http:"; } catch { return false; } }

...
export function StockEarningsExpectationPanel(props: StockEarningsExpectationPanelProps) {
  ...
  if (!snapshots.length) return <div className="rounded-lg border border-borderSoft bg-bg2/60 p-4"><p className="text-sm text-textMuted">{props.providerLoadStatus === "loading" ? "公司官方指引按需加载中。" : props.providerLoadStatus === "error" ? `公司指引 Provider 加载失败：${props.providerLoadError ?? "未知错误"}` : "当前公司尚无可靠公司指引或用户业绩预期快照。Provider 不会生成示例预测，也不会写入 LocalStorage。"}</p>{props.onAdd ? <button type="button" onClick={() => props.onAdd?.(props.stock)} className={`${buttonClass} mt-3 border-cyan/50 text-cyan`}><Plus className="h-4 w-4" />添加业绩预期</button> : null}</div>;

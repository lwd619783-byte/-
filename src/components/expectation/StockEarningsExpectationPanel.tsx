...
import { getExpectationBusinessTime, selectEffectiveEarningsExpectations, sortExpectationsByBusinessTime } from "../../services/earningsExpectationIntegrity";
...
export function StockEarningsExpectationPanel(props: StockEarningsExpectationPanelProps) {
  ...
  const selections = selectEffectiveEarningsExpectations(snapshots, props.timeZone);
  ...
  const comparisons = buildEarningsExpectationComparisons(snapshots, actualEvents, { revisionReminderThreshold: 0.1, nearZeroThreshold: 1e-9, roundingTolerance: 1e-9, timeZone: props.timeZone ?? Intl.DateTimeFormat().resolvedOptions().timeZone ?? "UTC" });

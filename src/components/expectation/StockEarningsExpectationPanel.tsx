...
import type { AShareAnnouncementData, AShareFinancialData, EarningsExpectationProviderSnapshot, EarningsExpectationSnapshot, Stock } from "../../types";
...
interface StockEarningsExpectationPanelProps {
  ...
  providerLoadStatus?: "idle" | "loading" | "success" | "partial" | "error";
...
export function StockEarningsExpectationPanel(props: StockEarningsExpectationPanelProps) {
  const localReadNoticeId = useId();
  const localReadNotice = props.localReadError ? <div role="alert" className="rounded-lg border border-warning/40 bg-warning/10 p-4 text-sm text-textStrong">
    <p id={localReadNoticeId}>本地预期暂不可读，范围不完整。本地记录未纳入当前展示；新增和纠正暂不可用，已读取的官方指引仍可查看。</p>
    <details className="mt-2 text-xs text-textMuted"><summary className="cursor-pointer">查看本地预期不可读原因</summary><p className="mt-2 break-words">{props.localReadError}</p></details>
  </div> : null;
  const localWriteDisabled = Boolean(props.localReadError);
  ...
  if (!snapshots.length) return <div className="space-y-4">{localReadNotice}<div className="rounded-lg border border-borderSoft bg-bg2/60 p-4"><p className="text-sm text-textMuted">{props.localReadError ? "当前没有可展示的已读取记录；本地记录数量未知。" : props.providerLoadStatus === "loading" ? "公司官方指引按需加载中。" : props.providerLoadStatus === "error" ? `公司指引数据加载失败：${props.providerLoadError ?? "未知错误"}` : "当前公司尚无可靠公司指引或用户业绩预期快照。数据提供方不会生成示例预测，也不会写入本地存储。"}</p>{props.localReadError && (props.providerLoadStatus === "loading" || props.providerLoadStatus === "error") ? <p className="mt-2 text-sm text-textMuted">{props.providerLoadStatus === "loading" ? "公司官方指引按需加载中。" : `公司指引数据加载失败：${props.providerLoadError ?? "未知错误"}`}</p> : null}{props.onAdd ? <button type="button" disabled={localWriteDisabled} aria-describedby={localWriteDisabled ? localReadNoticeId : undefined} onClick={() => props.onAdd?.(props.stock)} className={`${buttonClass} mt-3 border-cyan/50 text-cyan`}><Plus className="h-4 w-4" />添加业绩预期</button> : null}</div></div>;
  return <div className="space-y-4">
    {localReadNotice}
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-semibold text-textStrong">当前有效预期</p><p className="mt-1 text-xs text-textMuted">公司指引、单家机构、机构一致预期和用户预测分别展示，不合并成单一标签。</p></div>{props.onAdd ? <button type="button" disabled={localWriteDisabled} aria-describedby={localWriteDisabled ? localReadNoticeId : undefined} onClick={() => props.onAdd?.(props.stock)} className={`${buttonClass} border-cyan/50 text-cyan`}><Plus className="h-4 w-4" />添加新快照</button> : null}</div>
    <div className="space-y-3">{effective.map((snapshot) => { const isProvider = props.providerSnapshotIds?.has(snapshot.id) ?? false; const providerRecord = props.providerRecordBySnapshotId?.get(snapshot.id); const duplicateOfProviderId = props.duplicateOfProviderByLocalId?.get(snapshot.id); const selection = selectionBySnapshot.get(snapshot.id); const comparison = comparisonBySnapshot.get(snapshot.id); const history = snapshots.filter((item) => expectationGroupKey(item) === expectationGroupKey(snapshot)); const businessNodes = resolveEffectiveBusinessHistory(history, timeZone); const currentNode = businessNodes.find((node) => node.effectiveSnapshot.id === snapshot.id); return <article key={snapshot.id} className="rounded-lg border border-borderSoft bg-bg2/60 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap gap-2 text-xs"><Badge value={isProvider ? "公司官方指引" : sourceCategoryLabel(snapshot.sourceCategory)} warning={snapshot.sourceCategory === "user_estimate"} /><Badge value={snapshot.sourceVerificationStatus} warning={snapshot.sourceVerificationStatus !== "verified"} />{isProvider ? <Badge value="Provider 只读" /> : null}{duplicateOfProviderId ? <Badge value="与官方 Provider 记录重复" warning /> : null}{snapshot.correctsSnapshotId ? <Badge value={snapshot.correctionScope === "basis" ? "口径纠正" : "数值纠正"} warning /> : null}{comparison?.isExAnte ? <Badge value="事前有效" /> : <Badge value="事后参考 / 待匹配" warning />}</div><p className="mt-2 break-words text-sm font-semibold text-textStrong">{metricLabel(snapshot.metric)} · {snapshot.reportPeriod} · {periodScopeLabel(snapshot.periodScope)}</p><p className="mt-1 break-words text-sm text-cyan">{formatSnapshot(snapshot)}</p></div><div className="flex flex-wrap gap-2">{snapshot.sourceUrl ? <a href={snapshot.sourceUrl} target="_blank" rel="noreferrer" className={buttonClass}><ExternalLink className="h-4 w-4" />巨潮官方公告</a> : null}{providerRecord?.officialPdfUrl ? <a href={providerRecord.officialPdfUrl} target="_blank" rel="noreferrer" className={buttonClass}>官方 PDF</a> : null}{props.onCorrect && !isProvider ? <button type="button" disabled={localWriteDisabled} aria-describedby={localWriteDisabled ? localReadNoticeId : undefined} onClick={() => props.onCorrect?.(snapshot)} className={buttonClass}>创建纠正快照</button> : null}</div></div>
      ...
      {uncertaintyBySnapshot.get(snapshot.id) && !selection?.previousResolution.reasonCode ? <div role="status" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">当前有效预测之间无法确认业务先后顺序；实际值比较与业务节点选择分别展示。</div> : null}
      {orderStatusBySnapshot.get(snapshot.id) === "equal" && !selection?.previousResolution.reasonCode ? <div role="status" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">当前有效预测形成于同一精确时刻；稳定 ID 仅用于显示，不代表业务先后。</div> : null}
      <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><Field label="来源名称" value={snapshot.sourceName || "缺失"} /><Field label="原记录业务时间 / 可用时间" value={formatBusinessTemporal(currentNode?.originalBusinessTime ?? getExpectationBusinessTime(snapshot, timeZone))} /><Field label="纠正后有效业务时间 / 可用时间" value={formatBusinessTemporal(currentNode?.effectiveBusinessTime ?? getExpectationBusinessTime(snapshot, timeZone))} /><Field label="当前有效来源时间" value={formatSourceTime(snapshot)} /><Field label="分析师 / 机构数量" value={`${snapshot.analystCount ?? "-"} / ${snapshot.institutionCount ?? "-"}`} /><Field label="会计口径" value={snapshot.accountingBasis} /></div>
      ...
      {history.length > 1 ? <div className="mt-3 rounded border border-borderSoft bg-surface/60 p-3"><p className="text-xs font-semibold text-textStrong">业务节点与纠正链历史（仅追加）</p>{businessNodes.map((node) => <div key={node.businessRootSnapshot.id} className="mt-2 border-l border-borderSoft pl-3 text-xs text-textMuted"><p>原记录可用时间 {formatBusinessTemporal(node.originalBusinessTime)} · 当前有效可用时间 {formatBusinessTemporal(node.effectiveBusinessTime)} · 根快照 {node.businessRootSnapshot.id} · 当前终点 {node.effectiveSnapshot.id} · {formatSnapshot(node.effectiveSnapshot)}</p>{node.correctionChain.slice(1).map((correction) => { const target = node.correctionChain.find((item) => item.id === correction.correctsSnapshotId); const delta = deriveExpectationCorrectionDelta(correction, target); return <p key={correction.id} className="mt-1 text-warning">纠正记录 {correction.createdAt} · {correction.correctsSnapshotId} → {correction.id} · 变化字段：{delta?.changedFields.join("、") || "待核验"} · 原因：{correction.notes ?? "缺失"} · 当前有效可用时间 {formatBusinessTemporal(node.effectiveBusinessTime)}</p>; })}</div>)}</div> : null}
    </article>; })}</div>
...
function Badge({ value, warning = false }: { value: string; warning?: boolean }) { return <span className={`rounded border px-2 py-1 ${warning ? "border-warning/40 bg-warning/10 text-warning" : "border-cyan/30 bg-cyan/10 text-cyan"}`}>{statusDisplayLabel(value)}</span>; }

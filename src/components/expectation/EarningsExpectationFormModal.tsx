  ...
  return (
    <Modal error={error} busy={pending} hasUnsavedChanges={dirty} onDiscard={onClose} title={correctionTarget ? "创建纠正快照" : "添加业绩预期"} description="保存后不可原地修改；错误修正必须追加纠正快照。" onClose={close} footer={<><button type="button" disabled={pending} onClick={close} className={buttonClass}>取消</button><button type="button" onClick={() => void run(submit)} disabled={pending || !selectedStock} className={`${buttonClass} border-cyan/50 text-cyan`}>{pending ? "保存中…" : correctionTarget ? "保存纠正快照" : "保存不可变快照"}</button></>}>
      <div className="grid min-w-0 gap-3 sm:grid-cols-2">
        ...
        <Field label="币种"><select value={form.currency} onChange={(event) => set("currency", event.target.value)} className={inputClass}><option>人民币（CNY）</option><option>港元（HKD）</option><option>美元（USD）</option></select></Field>
        ...
        <Field label="会计口径"><select value={form.accountingBasis} onChange={(event) => set("accountingBasis", event.target.value)} className={inputClass}><option value="PRC_GAAP">中国企业会计准则</option><option value="IFRS">国际财务报告准则（IFRS）</option><option value="unknown">未知（将不可比较）</option></select></Field>
      ...
      <div className="mt-4 rounded border border-borderSoft bg-surface/60 p-3 text-xs leading-5 text-textMuted">工作流时区：<span className="font-semibold text-textStrong">{timeZone}</span>。日期校验与本地日期时间转换均使用该时区。</div>

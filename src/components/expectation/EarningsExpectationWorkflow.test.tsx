...
describe("earnings expectation UI and workflow linkage", () => {
  ...
  it("65 confines wide comparison content to an accessible local scroll region", () => { const html = centerHtml([snapshot()]); expect(html).toContain("min-w-0"); expect(html).toContain('<section class="min-w-0 space-y-4"'); expect(html).toContain('class="mt-4 max-w-full overflow-x-auto" tabindex="0" role="region" aria-label="同口径来源对照表，可横向滚动"'); expect((html.match(/overflow-x-auto/g) ?? [])).toHaveLength(1); });
  ...
  it("does not fabricate midnight for a date-only business time", () => { const html = centerHtml([snapshot()]); expect(html).toContain("2026-06-01 (日期)"); expect(html).not.toContain("2026-06-01T00:00"); });

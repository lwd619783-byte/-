...
export function EarningsExpectationCenter(props: EarningsExpectationCenterProps) {
  ...
  return (
    <section className="min-w-0 space-y-4" aria-label="业绩预期证据中心">
      ...
      <DashboardCard className="p-4" aria-label="公司指引 Provider 状态">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.16em] text-cyan">Company Guidance Provider V2</p><h2 className="mt-1 text-base font-semibold text-textStrong">公司官方指引 · 巨潮官方公告</h2><p className="mt-1 text-xs leading-5 text-textMuted">全局工作流索引与页面导航解耦；Provider 记录只读，不写入用户 LocalStorage，也不覆盖人工、JSON 或 CSV 快照。</p></div><div className="text-right text-xs text-textMuted"><p>全局状态：{props.providerLoadStatus === "loading" ? "索引校验中" : props.providerLoadStatus === "error" ? "已关闭（校验失败）" : props.providerLoadStatus === "success" ? "已验证并启用" : "未启用"}</p><p className="mt-1">明细状态：{props.providerDetailLoadStatus ?? "idle"} · 成功 {props.providerLoadedCompanyCount ?? 0} / 失败 {props.providerFailedStockIds?.length ?? 0}</p><p className="mt-1">版本：{props.providerSummary?.providerVersion ?? "-"} · 更新：{props.providerSummary?.generatedAt ?? "-"}</p></div></div>
        {props.providerLoadError ? <p role="alert" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">Provider 全局索引加载或校验失败：{props.providerLoadError}。正式 Provider 已全局关闭，本地快照仍可使用。</p> : null}
        {props.providerDetailLoadError ? <p role="alert" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">明细部分失败：{props.providerDetailLoadError}。成功公司仍保留，失败结果不缓存。</p> : null}
        {(props.providerLoadError || props.providerDetailLoadError) && props.onRetryProvider ? <button type="button" onClick={props.onRetryProvider} className={`${buttonClass} mt-3`}>重试 Provider</button> : null}
        <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5"><Value label="全局 Provider 快照" value={String(kpis.providerSnapshots)} /><Value label="已加载公司" value={String(kpis.providerCompanies)} /><Value label="本地重复/元数据差异" value={String(kpis.providerDuplicates)} /><Value label="内容冲突" value={String(kpis.providerConflicts)} /><Value label="排除 / 警告" value={`${props.providerExclusions?.length ?? 0} / ${props.providerWarnings?.length ?? 0}`} /></div>
        {props.duplicateOfProviderByLocalId?.size ? <div role="status" className="mt-3 rounded border border-warning/35 bg-warning/10 p-3 text-xs text-warning"><p className="font-medium text-textStrong">与官方 Provider 记录重复 / 元数据不同</p><p className="mt-1">保留 {props.duplicateOfProviderByLocalId.size} 条本地记录用于审计；Comparison、ResearchEvent 与 ReviewTask 只计对应官方 Provider 版本。</p></div> : null}
        {kpis.providerConflicts ? <div role="alert" className="mt-3 rounded border border-warning/50 bg-warning/10 p-3 text-xs text-warning">发现 {kpis.providerConflicts} 条同一官方证据的财务内容冲突；本地记录保留可见，但不进入比较链，需按冲突字段复核。</div> : null}
        {props.providerRelationByLocalId?.size ? <div className="mt-3 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="text-textMuted"><tr><th className="p-2">本地快照</th><th className="p-2">Provider 关系</th><th className="p-2">Provider 版本</th><th className="p-2">差异字段</th></tr></thead><tbody>{[...props.providerRelationByLocalId.values()].filter((record) => record.relation !== "independent").map((record) => <tr key={record.localSnapshotId} className="border-t border-borderSoft"><td className="break-all p-2">{record.localSnapshotId}</td><td className="p-2">{record.relation}</td><td className="break-all p-2">{record.providerSnapshotId ?? "-"}</td><td className="p-2">{record.conflictingFields.join("、") || "无"}</td></tr>)}</tbody></table></div> : null}
      </DashboardCard>
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.55fr)]">
        <DashboardCard className="min-w-0 p-4"><h2 className="text-base font-semibold text-textStrong">有效快照、业务修订与数据更正时间线</h2><div className="mt-4 space-y-3">{filtered.length ? filtered.map((snapshot) => <SnapshotCard key={snapshot.id} snapshot={snapshot} selection={selectionBySnapshot.get(snapshot.id)} comparison={comparisonBySnapshot.get(snapshot.id)} history={props.snapshots.filter((item) => expectationGroupKey(item) === expectationGroupKey(snapshot))} stock={props.stocks.find((item) => item.id === snapshot.stockId)} industries={props.industries} watched={activeWatchStocks.has(snapshot.stockId)} businessOrderStatus={orderStatusBySnapshot.get(snapshot.id) ?? (uncertaintyBySnapshot.get(snapshot.id) ? "uncertain" : "confirmed")} timeZone={timeZone} isProvider={props.providerSnapshotIds?.has(snapshot.id) ?? false} duplicateOfProviderId={props.duplicateOfProviderByLocalId?.get(snapshot.id)} providerRecord={props.providerRecordBySnapshotId?.get(snapshot.id)} onCorrect={props.onCorrect} onOpenStock={props.onOpenStock} />) : <EmptyState title="没有匹配的业绩预期" description="请调整筛选，或添加一条有明确来源和形成时间的快照。" />}</div></DashboardCard>
        <DashboardCard className="min-w-0 p-4"><h2 className="text-base font-semibold text-textStrong">数据核验队列</h2><p className="mt-1 text-xs text-textMuted">来源、日期、口径、单位、实际值、业务前序或解析状态不足时明确保留。</p><div className="mt-4 space-y-3">{queueSnapshots.map((snapshot) => { const comparison = comparisonBySnapshot.get(snapshot.id); const previousResolution = selectionBySnapshot.get(snapshot.id)?.previousResolution; const businessOrderIssue = previousResolution && ["ambiguous", "equal_time", "unresolved"].includes(previousResolution.status); return <article key={`queue-${snapshot.id}`} className="min-w-0 rounded border border-warning/35 bg-warning/10 p-3"><p className="break-words text-sm font-medium text-textStrong">{props.stocks.find((stock) => stock.id === snapshot.stockId)?.name ?? snapshot.stockId} · {snapshot.reportPeriod}</p><p className="mt-1 break-words text-xs text-warning">{businessOrderIssue ? `上一业务预测无法唯一确认，候选 ${previousResolution.candidateNodes.length} 条；不计算上修或下修。` : snapshot.sourceVerificationStatus !== "verified" ? `来源状态：${snapshot.sourceVerificationStatus}` : comparison?.nonComparableReasons.join("；") || "无法匹配实际值"}</p></article>; })}{props.providerExclusions?.slice(0, 30).map((item) => <article key={`provider-exclusion-${item.sourceAnnouncementId}-${item.metric ?? "none"}`} className="rounded border border-warning/35 bg-warning/10 p-3"><p className="text-sm text-textStrong">Provider 排除 · {item.companyName} · {item.sourceTitle}</p><p className="mt-1 break-words text-xs text-warning">{item.reasons.join("；")} · 不生成数值、不补 0</p></article>)}{props.providerWarnings?.map((item) => <article key={`provider-warning-${item.sourceAnnouncementId}-${item.code}`} className="rounded border border-warning/35 bg-warning/10 p-3"><p className="text-sm text-textStrong">Provider 警告 · [{item.code}]</p><p className="mt-1 break-words text-xs text-warning">{item.message} 候选公告：{item.candidateAnnouncementIds.join("、") || "无"}</p></article>)}{importIssues.map(({ record, issue }, index) => <article key={`${record.id}-${index}`} className="rounded border border-warning/35 bg-warning/10 p-3"><p className="text-sm text-textStrong">{record.ingestionMethod === "csv_import" ? "CSV" : "JSON"} 导入核验 · 第 {issue.row} 行 · [{issue.code}]</p><p className="mt-1 break-words text-xs text-warning">{issue.message}</p></article>)}{!queueSnapshots.length && !importIssues.length && !props.providerExclusions?.length && !props.providerWarnings?.length ? <p className="text-sm text-textMuted">当前没有数据核验项。</p> : null}</div></DashboardCard>
      </div>
    </section>
  );
}

function SnapshotCard({ snapshot, selection, comparison, history, stock, industries, watched, businessOrderStatus, timeZone, isProvider, duplicateOfProviderId, providerRecord, onCorrect, onOpenStock }: { snapshot: EarningsExpectationSnapshot; selection?: EarningsExpectationSelection; comparison?: EarningsExpectationComparison; history: EarningsExpectationSnapshot[]; stock?: Stock; industries: Industry[]; watched: boolean; businessOrderStatus: "confirmed" | "equal" | "uncertain"; timeZone?: string; isProvider: boolean; duplicateOfProviderId?: string; providerRecord?: EarningsExpectationProviderSnapshot; onCorrect: (snapshot: EarningsExpectationSnapshot) => void; onOpenStock: (stock: Stock) => void }) {
  ...
  return <article className="min-w-0 rounded-lg border border-borderSoft bg-bg2/65 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="break-words text-xs text-textMuted">{stock?.name ?? snapshot.stockId} · {stock?.code ?? ""} · {stock ? getIndustryName(industries, stock.industryId) : "行业缺失"}</p><h3 className="mt-1 break-words text-base font-semibold text-textStrong">{metricLabel(snapshot.metric)} · {snapshot.reportPeriod} · {periodScopeLabel(snapshot.periodScope)}</h3><div className="mt-2 flex flex-wrap gap-2 text-xs"><Badge value={isProvider ? "公司官方指引" : sourceCategoryLabel(snapshot.sourceCategory)} warning={snapshot.sourceCategory === "user_estimate"} /><Badge value={snapshot.sourceVerificationStatus} warning={snapshot.sourceVerificationStatus !== "verified"} />{isProvider ? <Badge value="Provider 只读" /> : null}{duplicateOfProviderId ? <Badge value="与官方 Provider 记录重复" warning /> : null}{watched ? <Badge value="观察清单" /> : null}{snapshot.correctsSnapshotId ? <Badge value={snapshot.correctionScope === "basis" ? "口径纠正" : "数值纠正"} warning /> : null}</div></div><div className="flex flex-wrap gap-2">{snapshot.sourceUrl ? <a href={snapshot.sourceUrl} target="_blank" rel="noreferrer" className={buttonClass}><ExternalLink className="h-4 w-4" />巨潮官方公告</a> : null}{providerRecord?.officialPdfUrl ? <a href={providerRecord.officialPdfUrl} target="_blank" rel="noreferrer" className={buttonClass}>官方 PDF</a> : null}{stock ? <button type="button" onClick={() => onOpenStock(stock)} className={buttonClass}>个股详情</button> : null}{!isProvider ? <button type="button" onClick={() => onCorrect(snapshot)} className={buttonClass}>创建纠正</button> : null}</div></div>
    ...
    <EarningsExpectationTemporalAudit snapshot={snapshot} selection={selection} comparison={comparison} displayTimeZone={timeZone ?? "UTC"} />
...
function formatBusinessTemporal(value: ReturnType<typeof getExpectationBusinessTime>) { return value ? `${value.value} (${value.precision})` : "不确定（未用形成时间回填）"; }

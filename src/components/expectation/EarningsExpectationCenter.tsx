...
export function EarningsExpectationCenter(props: EarningsExpectationCenterProps) {
  ...
  return (
    <section className="min-w-0 space-y-4" aria-label="业绩预期证据中心">
      ...
      <DashboardCard className="p-4" aria-label="公司指引数据状态">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs tracking-[0.16em] text-cyan">公司指引数据 · 第二版</p><h2 className="mt-1 text-base font-semibold text-textStrong">公司官方指引 · 巨潮官方公告</h2><p className="mt-1 text-xs leading-5 text-textMuted">全局工作流索引与页面导航解耦；数据提供方记录只读，不写入用户本地存储，也不覆盖人工、JSON 或 CSV 快照。</p></div><div className="text-right text-xs text-textMuted"><p>全局状态：{props.providerLoadStatus === "loading" ? "索引校验中" : props.providerLoadStatus === "error" ? "已关闭（校验失败）" : props.providerLoadStatus === "success" ? "已验证并启用" : "未启用"}</p><p className="mt-1">明细状态：{statusDisplayLabel(props.providerDetailLoadStatus ?? "idle")} · 成功 {props.providerLoadedCompanyCount ?? 0} / 失败 {props.providerFailedStockIds?.length ?? 0}</p><p className="mt-1">版本：{props.providerSummary?.providerVersion ?? "-"} · 更新：{props.providerSummary?.generatedAt ?? "-"}</p></div></div>
        {props.providerLoadError ? <p role="alert" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">全局索引加载或校验失败：{props.providerLoadError}。正式数据提供方已全局关闭，本地快照仍可使用。</p> : null}
        ...
        {(props.providerLoadError || props.providerDetailLoadError) && props.onRetryProvider ? <button type="button" onClick={props.onRetryProvider} className={`${buttonClass} mt-3`}>重试数据加载</button> : null}
        <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-5"><Value label="全局数据快照" value={String(kpis.providerSnapshots)} /><Value label="已加载公司" value={String(kpis.providerCompanies)} /><Value label="本地重复/元数据差异" value={String(kpis.providerDuplicates)} /><Value label="内容冲突" value={String(kpis.providerConflicts)} /><Value label="排除 / 警告" value={`${props.providerExclusions?.length ?? 0} / ${props.providerWarnings?.length ?? 0}`} /></div>
        {props.duplicateOfProviderByLocalId?.size ? <div role="status" className="mt-3 rounded border border-warning/35 bg-warning/10 p-3 text-xs text-warning"><p className="font-medium text-textStrong">与官方数据记录重复 / 元数据不同</p><p className="mt-1">保留 {props.duplicateOfProviderByLocalId.size} 条本地记录用于审计；比较、研究事件与复盘任务只计对应官方数据版本。</p></div> : null}

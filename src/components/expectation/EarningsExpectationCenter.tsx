...
import type { CompanyGuidanceExpectationExclusion, CompanyGuidanceExpectationSummary, CompanyGuidanceExpectationWarning, EarningsExpectationComparison, EarningsExpectationImportRecord, EarningsExpectationProviderSnapshot, EarningsExpectationSnapshot, Industry, ResearchEvent, Stock, WatchItem } from "../../types";
...
interface EarningsExpectationCenterProps {
  ...
  providerLoadStatus?: "idle" | "loading" | "success" | "error";
...
export function EarningsExpectationCenter(props: EarningsExpectationCenterProps) {
  ...
  return (
    <section className="min-w-0 space-y-4" aria-label="业绩预期证据中心">
      ...
      <DashboardCard className="p-4" aria-label="公司指引 Provider 状态"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.16em] text-cyan">Company Guidance Provider V1</p><h2 className="mt-1 text-base font-semibold text-textStrong">公司官方指引 · 巨潮官方公告</h2><p className="mt-1 text-xs leading-5 text-textMuted">只读取已提交公告产物；Provider 记录只读，不写入用户 LocalStorage，也不覆盖人工、JSON 或 CSV 快照。</p></div><div className="text-right text-xs text-textMuted"><p>状态：{props.providerLoadStatus === "loading" ? "按公司加载中" : props.providerLoadStatus === "error" ? "加载失败" : props.providerSnapshotIds?.size ? "已加载真实数据" : "Provider 数据暂缺 / 尚未加载"}</p><p className="mt-1">版本：{props.providerSummary?.providerVersion ?? "-"} · 更新：{props.providerSummary?.generatedAt ?? "-"}</p></div></div>{props.providerLoadError ? <p role="alert" className="mt-3 rounded border border-warning/40 bg-warning/10 p-2 text-xs text-warning">Provider 加载失败：{props.providerLoadError}。失败结果未缓存为成功。</p> : null}<div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><Value label="已加载 Provider 快照" value={String(kpis.providerSnapshots)} /><Value label="已加载公司" value={String(kpis.providerCompanies)} /><Value label="本地重复记录" value={String(kpis.providerDuplicates)} /><Value label="排除 / 警告" value={`${props.providerExclusions?.length ?? 0} / ${props.providerWarnings?.length ?? 0}`} /></div>{props.duplicateOfProviderByLocalId?.size ? <div role="status" className="mt-3 rounded border border-warning/35 bg-warning/10 p-3 text-xs text-warning"><p className="font-medium text-textStrong">与官方 Provider 记录重复</p><p className="mt-1">保留 {props.duplicateOfProviderByLocalId.size} 条本地记录用于审计；Comparison、ResearchEvent 与 ReviewTask 仅使用对应官方 Provider 快照，避免重复计数。</p></div> : null}</DashboardCard>
...
function SnapshotCard({ snapshot, selection, comparison, history, stock, industries, watched, businessOrderStatus, timeZone, isProvider, duplicateOfProviderId, providerRecord, onCorrect, onOpenStock }: { snapshot: EarningsExpectationSnapshot; selection?: EarningsExpectationSelection; comparison?: EarningsExpectationComparison; history: EarningsExpectationSnapshot[]; stock?: Stock; industries: Industry[]; watched: boolean; businessOrderStatus: "confirmed" | "equal" | "uncertain"; timeZone?: string; isProvider: boolean; duplicateOfProviderId?: string; providerRecord?: EarningsExpectationProviderSnapshot; onCorrect: (snapshot: EarningsExpectationSnapshot) => void; onOpenStock: (stock: Stock) => void }) {
  ...
  return <article className="min-w-0 rounded-lg border border-borderSoft bg-bg2/65 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="break-words text-xs text-textMuted">{stock?.name ?? snapshot.stockId} · {stock?.code ?? ""} · {stock ? getIndustryName(industries, stock.industryId) : "行业缺失"}</p><h3 className="mt-1 break-words text-base font-semibold text-textStrong">{metricLabel(snapshot.metric)} · {snapshot.reportPeriod} · {periodScopeLabel(snapshot.periodScope)}</h3><div className="mt-2 flex flex-wrap gap-2 text-xs"><Badge value={isProvider ? "公司官方指引" : sourceCategoryLabel(snapshot.sourceCategory)} warning={snapshot.sourceCategory === "user_estimate"} /><Badge value={snapshot.sourceVerificationStatus} warning={snapshot.sourceVerificationStatus !== "verified"} />{isProvider ? <Badge value="Provider 只读" /> : null}{duplicateOfProviderId ? <Badge value="与官方 Provider 记录重复" warning /> : null}{watched ? <Badge value="观察清单" /> : null}{snapshot.correctsSnapshotId ? <Badge value={snapshot.correctionScope === "basis" ? "口径纠正" : "数值纠正"} warning /> : null}</div></div><div className="flex flex-wrap gap-2">{snapshot.sourceUrl ? <a href={snapshot.sourceUrl} target="_blank" rel="noreferrer" className={buttonClass}><ExternalLink className="h-4 w-4" />巨潮官方公告</a> : null}{providerRecord?.officialPdfUrl ? <a href={providerRecord.officialPdfUrl} target="_blank" rel="noreferrer" className={buttonClass}>官方 PDF</a> : null}{stock ? <button type="button" onClick={() => onOpenStock(stock)} className={buttonClass}>个股详情</button> : null}{!isProvider ? <button type="button" onClick={() => onCorrect(snapshot)} className={buttonClass}>创建纠正</button> : null}</div></div>
    {isProvider ? <div role="status" className="mt-3 rounded border border-cyan/35 bg-cyan/10 p-2 text-xs text-cyan">公司内部形成时间未知，以公开披露时间作为可用时间。Provider {providerRecord?.providerVersion ?? snapshot.providerVersion ?? "-"} · 更新 {providerRecord?.generatedAt ?? snapshot.providerGeneratedAt ?? "-"} · 公告 ID {providerRecord?.sourceAnnouncementId ?? snapshot.sourceAnnouncementId ?? "-"}</div> : null}

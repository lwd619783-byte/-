  ...
  return <Modal title={titles[kind]} description="留空即 unknown。新增与修订均追加保存；审核不验证观点正确性。" onClose={close} onDiscard={onClose} hasUnsavedChanges={dirty} error={error}>
    <form className="space-y-4" onSubmit={submit} onChange={() => setDirty(true)}>
      ...
      {kind === 'event' && <>
        ...
        <Select label="核验状态" name="verificationStatus" value="unverified"><option value="unverified">unverified · 未核验</option><option value="partial">partial · 部分核验</option><option value="verified">verified · 事件来源已核验</option></Select><p className="text-xs text-warning">事件 verified 仅表示事件来源级核验，需要可核对的来源 URL；不代表 Provider Fact 准入、Verified Claim 或正式 Thesis。</p>
      ...
      {kind === 'review' && due && <><p className="text-sm">T+{due.offsetDays} · calendar days · 来源发布时间锚点 {due.anchorAt ?? 'unknown'} · 到期 {due.dueAt ?? 'unresolved · 不可计算'}</p><p className="text-sm">原观点：{observation?.summary ?? 'unknown'}</p><p className="text-sm">成立条件：{observation?.trigger ?? 'unknown'}；失效条件：{observation?.invalidation ?? 'unknown'}</p><p className="text-xs text-warning">复盘按来源发布时间加日历日计算；不会用录入或审核时间替代。未知锚点或尚未到期仅可保存 pending。</p><Select label="复盘状态" name="status" value={due.dueAt && new Date(due.dueAt).getTime() <= Date.now() ? 'completed' : 'pending'}><option value="completed">completed · 已完成</option><option value="inconclusive">inconclusive · 无法判断</option><option value="pending">pending · 待复盘</option></Select><Select label="成立条件是否触发" name="triggerOccurred" value={due.result?.triggerOccurred ?? 'unknown'}>{yesNoUnknown}</Select><Select label="失效条件是否触发" name="invalidationOccurred" value={due.result?.invalidationOccurred ?? 'unknown'}>{yesNoUnknown}</Select><Field name="actualOutcome" label="后续实际表现" area value={due.result?.actualOutcome ?? ''} /><Field name="evidence" label="后续验证证据" area value={due.result?.evidence ?? ''} /><Field name="url" label="证据 URL" type="url" value={due.result?.evidenceUrl ?? ''} /></>}

...
export function CreatorViewpointWorkspace({ repository: providedRepository, readOnly = false }: { repository?: CreatorViewpointRepository; readOnly?: boolean }) {
  ...
  const stateCard = (state: ViewpointCurrent | undefined) => state ? <>
    ...
    <p className="mt-3 text-xs text-textMuted">Creator time {state.effectiveAt}<br />本地审核 {state.reviewedAt}<br />来源 / 评论覆盖 {state.coverage}</p>
    <p className="mt-2 text-xs text-textMuted">最近变化：{state.lastTransition ? `${state.lastTransition.previous ? `${stanceLabels[state.lastTransition.previous.stance]} → ${stanceLabels[state.lastTransition.next.stance]}` : '首次建立状态'} · Creator time ${state.lastTransition.effectiveAt} · 本地审核 ${state.lastTransition.recordedAt}` : '无状态变化'}</p>
  ...
  </> : <p className="mt-3 text-sm text-textMuted">尚无具备可靠 Creator time 的已审核观点 · unknown / unresolved</p>;
  const observationCard = (observation: ViewpointObservation) => {
    ...
    return <article className="rounded border border-borderSoft bg-bg1 p-4" key={observation.id}>
      <div className="flex flex-wrap items-start justify-between gap-2"><p className="text-xs text-textMuted">{data.creators.find(item => item.id === observation.creatorId)?.name} / {data.topics.find(item => item.id === observation.topicId)?.name}<br />Creator time {effectiveAt ?? 'unresolved · 来源时间未知'}<br />本地记录 {observation.recordedAt}<br />本地审核 {approval?.recordedAt ?? '尚未审核'}</p><span className="text-xs text-warning">{status}{superseded ? ' · 已有后续修订，原历史保留' : ''}</span></div>
      <div className="mt-3 space-y-3 border-l-2 border-borderSoft pl-3">
        ...
        <div><p className="text-xs text-textMuted">04 状态历史</p><p className="text-sm">{transition ? `${transition.previous ? `${stanceLabels[transition.previous.stance]} → ${stanceLabels[transition.next.stance]}` : '首次建立状态'} · Creator time ${transition.effectiveAt} · 本地审核 ${transition.recordedAt}` : status === 'draft' ? 'Draft · 不改变正式 Current View' : status === 'rejected' ? '记录已拒绝 · 不改变 Current View' : superseded ? '已被审核修订替代 · 原历史保留，不参与当前状态' : chronology === 'ambiguous_time' ? 'unresolved · 同一 Creator time 存在不同状态，时序未证明' : !effectiveAt ? 'unresolved · 来源时间未知，不参与有序状态转换，可能影响 Current View 完整性' : '状态未改变 · 新增观点记录'}</p>{transition?.previous && <p className="mt-1 whitespace-pre-wrap break-words text-sm text-cyan">{stateChanges(transition.previous, transition.next)}</p>}{observation.supersedesId && <p className="text-xs text-warning">追加修订 · {observation.revisionReason ?? 'unknown'}</p>}</div>
        ...
        <div><p className="text-xs text-textMuted">06 后续 Review · 日历日</p>{reviews.filter(item => item.observationId === observation.id).map(item => <div key={item.offsetDays} className="mt-2 flex flex-wrap items-center gap-2 text-xs"><span>T+{item.offsetDays} · 来源锚点 {item.anchorAt ?? 'unknown'} · 到期 {item.dueAt?.slice(0, 10) ?? 'unresolved · 到期日不可计算'} · {item.result?.status === 'inconclusive' ? 'inconclusive · 待继续核验' : item.result?.status ?? 'pending'}</span>{item.result && <button type="button" className="inbox-action" onClick={() => setEvidence({ observation, review: item.result!, asOf })}>复盘证据</button>}<button type="button" className="inbox-action" disabled={blocked || !!asOf} onClick={() => setEntry({ kind: 'review', observation, due: item })}>{item.result ? '追加复盘修订' : '填写复盘'}</button></div>)}{!reviews.some(item => item.observationId === observation.id) && <p className="text-sm text-textMuted">审核后的重要观点提供 T+5 / T+20 / T+60 复盘</p>}</div>
      </div>
  ...
  return <section className="min-w-0 space-y-4 text-textStrong" aria-label="观点追踪工作区">
    ...
    {view === 'timeline' && <><div className="grid gap-3 md:grid-cols-2"><label className="text-sm">筛选博主<select aria-label="筛选博主" className={selectClass} value={creatorId} onChange={event => setCreatorId(event.target.value)}><option value="">全部博主</option>{creators.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-sm">筛选主题<select aria-label="筛选主题" className={selectClass} value={topicId} onChange={event => setTopicId(event.target.value)}><option value="">全部主题</option>{topics.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div><div className="space-y-3">{observations.filter(item => (!creatorId || item.creatorId === creatorId) && (!topicId || item.topicId === topicId)).map(observationCard)}</div>{!observations.length && <p className="p-4 text-sm text-textMuted">暂无观点记录。新增来源后记录观点，Draft 会在时间轴中保留。</p>}</>}

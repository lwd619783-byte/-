...
export function Header({onHome,onStartResearch,workspaceSearch,search,onSearchChange,updatedAt,sourceNote,dataMode,modeLabel,coverageSummary,onDataModeChange}: HeaderProps) {
  ...
  return <header className="workspace-header">
    <div className="workspace-topbar">
      <a className="workspace-brand" href="#/home" onClick={event => { if(onHome){event.preventDefault();onHome();} }}><span aria-hidden="true">◈</span> 投研 OS</a>
      {workspaceSearch ?? <label className="workspace-search"><Search aria-hidden="true" className="h-4 w-4 shrink-0 text-cyan" />
      ...
      <div className="ui-v2-header-actions"><span className="ui-v2-local-label">本地研究工作区</span>
    ...
    <details className="workspace-data-status">
      <summary aria-label="数据来源与时点"><StatusBadge status={modeStatus} label={`当前模式：${displayModeLabel}`} /><span className="text-textMuted">数据来源与时点</span></summary>

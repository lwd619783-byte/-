...
export function StockWatchlistPanel({ activeItem, archivedItem, tasks, entries, events, onAdd, onEdit, onStartReview, onCorrectReview, onRestore }: StockWatchlistPanelProps) {
  ...
  return <div className="space-y-4">
    ...
    {pending.length ? <div className="space-y-2"><h4 className="text-sm font-semibold text-textStrong">当前待处理复盘任务</h4>{pending.map((task) => <article key={task.id} className="rounded border border-warning/30 bg-warning/10 p-3"><p className="text-sm text-textStrong">{task.title}</p><p className="mt-1 break-words text-xs text-textMuted">{task.description}</p>{task.relatedEventIds.map((id) => events.find((event) => event.id === id)).filter((event): event is ResearchEvent => Boolean(event)).map((event) => <div key={event.id} className="mt-2 text-xs"><p className="text-textMuted">{event.expectation?.ingestionMethod === "provider" ? `公司官方指引 · 数据版本 ${event.expectation.providerVersion ?? "-"} · 只读` : event.sourceName}</p>{event.sourceUrl || event.pdfUrl ? <a href={event.sourceUrl ?? event.pdfUrl ?? undefined} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 break-all text-cyan"><ExternalLink className="h-3 w-3" />{event.expectation?.ingestionMethod === "provider" ? "巨潮官方公告" : event.sourceName}</a> : null}</div>)}</article>)}</div> : null}

...
export function WatchlistTab(props: WatchlistTabProps) {
  ...
  return <section className="min-w-0 space-y-4" aria-label="观察清单与投研复盘工作流">
    ...
    {rows.length === 0 && (props.storageError || props.corruptedRaw) ? null : rows.length === 0 ? <EmptyState title="没有匹配的用户观察项" description={hasFilters ? "当前条件下没有匹配记录，可清除筛选后继续查看。" : unmatched.length ? "原观察记录仍在，但当前研究池没有可匹配公司；可从上方导出备份。" : "尚未添加用户观察项。可以添加公司，或主动载入下方示例模板。"} /> : <div className={`grid min-w-0 items-start gap-4 ${selected ? "xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]" : ""}`}>

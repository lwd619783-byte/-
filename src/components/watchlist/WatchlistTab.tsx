      ...
      {rows.map(({ item, stock }) => (
        <article key={item.id} className="rounded-lg border border-line bg-white p-4 shadow-soft">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            ...
            <span className="rounded border border-line bg-panel px-2 py-1 text-xs">状态：{item.status}</span>
          </div>
          ...
          <div className="mt-4 rounded-md border border-line bg-panel p-3">
            <p className="text-xs font-semibold text-slate-500">需要验证的问题</p>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              className="h-9 rounded-md border border-line px-3 text-sm font-medium text-ink transition hover:border-signal hover:text-signal"

...
export function IndustryMetricPanel({ industryId }: { industryId: string }) {
  ...
  return <section className="rounded-lg border border-borderSoft bg-panel p-4" aria-label="正式行业指标"><h2 className="font-semibold text-textStrong">正式行业指标</h2><p className="mt-2 break-words text-sm text-textMuted">{!state ? '正在校验指标 Registry…' : state.status === 'blocked' ? `正式指标不可用 / blocked：${state.reason}` : '未实现 / not_implemented：当前行业没有正式 Industry Metric owner。'}</p></section>;
}
function MetricSelection({ metrics }: { metrics: RegisteredIndustryMetric[] }) {
  ...
  return <div className="min-w-0 space-y-3">
    ...
    {selected ? <MetricContent key={metricId} owner={selected.owner} entry={selected.entry} /> : <p>指标缺失 / missing</p>}
  </div>;
}
...
export function MetricContent({ owner, entry }: { owner: IndustryMetricDataset; entry?: RegisteredIndustryMetric['entry'] }) {
  ...
  return <section className="min-w-0 space-y-3" aria-label="正式行业指标">
    <div className="rounded-lg border border-borderSoft bg-panel p-4">
      ...
      <dl className="mt-3 grid gap-4 sm:grid-cols-3"><div><dt className="text-xs text-textMuted">留存末期 · {view.latest?.period ?? '暂缺'}</dt><dd className="mt-1 text-xl tabular-nums text-textStrong">{display(view.latest?.value)} {d.unit}</dd></div>{showDelta ? <div><dt className="text-xs text-textMuted">相邻月留存值差额</dt><dd className="mt-1 text-xl tabular-nums text-textStrong">{basis === 'monthly' ? `${display(view.delta)} ${d.unit}` : '不比较累计口径'}</dd></div> : null}<div><dt className="text-xs text-textMuted">声明窗口数值覆盖</dt><dd className="mt-1 text-xl tabular-nums text-textStrong">{view.available} / {view.target}</dd></div></dl>

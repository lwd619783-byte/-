...
const tabs = ['原始资料', 'AI 整理', '研究桥', '待审核', '我的知识库'] as const;
export type KnowledgeReviewState = { status: 'loading' | 'ready' | 'error' | 'locked'; count?: number; message?: string };
type Tab = typeof tabs[number];
type Props = WikiLibraryProps & { requestedAddMaterial?: boolean; onReviewStateChange?: (state: KnowledgeReviewState) => void; sourceRepository?: BrowserSourceRepository; requestedView?: Tab; routeKey?: string; onViewChange?: (view: Tab) => void };
export function ResearchMemoryWorkspace(props: Props) {
  ...
  const [addMaterial, setAddMaterial] = useState(false), [sourceLoadError, setSourceLoadError] = useState('');
  useEffect(() => { if (props.active !== false && props.requestedAddMaterial) setAddMaterial(true); }, [props.active, props.requestedAddMaterial, props.routeKey]);
  const addMaterialTrigger = useRef<HTMLButtonElement>(null), wasAdding = useRef(false);
  useEffect(() => { if (wasAdding.current && !addMaterial) addMaterialTrigger.current?.focus({ preventScroll: true }); wasAdding.current = addMaterial; }, [addMaterial]);
  ...
  const refresh = async () => { try { adopt(await sources.load()); setEpoch(e => e + 1); setSourceLoadError(''); setError(''); } catch (cause) { stateRef.current = null; setSnapshot(null); setSourceLoadError(String(cause)); setError(String(cause)); } };
  ...
  const saveFiles = (files: readonly File[]) => act(async () => {
    ...
    await sources.parseBatch(batch.batchId); await refresh(); setPaste(''); setAddMaterial(false); setMessage(`已保存 ${files.length} 份原件。可在“AI 整理”中交给 ChatGPT，或导出研究任务。`);
  });
  ...
  const reviewState: KnowledgeReviewState = !snapshot ? (sourceLoadError ? { status: 'error', message: sourceLoadError } : { status: 'loading' }) : loadedWiki.error ? { status: 'locked', message: loadedWiki.error } : wikiRead.error ? { status: 'error', message: wikiRead.error } : { status: 'ready', count: pending.length };
  const reviewStateCallback = useRef(props.onReviewStateChange); reviewStateCallback.current = props.onReviewStateChange;
  useEffect(() => { reviewStateCallback.current?.(reviewState); }, [reviewState.status, reviewState.count, reviewState.message]);
  ...
  return <section aria-label="研究记忆工作区" className="min-w-0 space-y-4 [overflow-wrap:anywhere]">
    ...
    {tab === '原始资料' && <>
      {snapshot && !snapshot.batches.length && <section className="memory-compact-empty" data-state="READY_EMPTY"><div><h3>还没有资料</h3><p>添加 PDF、Markdown 或 TXT；原件保存在当前浏览器，不会自动上传。</p></div><button ref={addMaterialTrigger} className="inbox-action" onClick={() => setAddMaterial(true)}>添加资料</button></section>}
      {!!snapshot?.batches.length && <div className="memory-list-toolbar"><p className="text-sm text-textMuted">原件保存在当前浏览器，不会自动上传。</p><button ref={addMaterialTrigger} className="inbox-action" onClick={() => setAddMaterial(true)}>添加资料</button></div>}

      {snapshot?.batches.slice().reverse().map(b => <section key={b.batchId} className="memory-source-batch"><h3 className="font-semibold">{b.title}</h3><p className="mt-1 text-xs text-textMuted">{b.sourceIds.length} 份资料 · {new Date(b.capturedAt).toLocaleString('zh-CN')}</p><ul className="memory-file-list">{snapshot.sources.filter(s => s.batchId === b.batchId).map(s => <li key={s.sourceId} className="memory-file-row"><div className="min-w-0"><button className="min-h-11 text-left text-cyan underline" onClick={() => setMaterial(s)}>{s.filename}</button><p className="text-xs text-textMuted">{(s.size / 1024).toFixed(1)} KiB · {{ pending: '等待解析', parsed: '文本已提取', failed: '解析失败，原件已保存' }[s.parse.status]}</p></div><button className={actionClass} disabled={busy} onClick={() => void act(async () => downloadWikiFile(await sources.readRaw(s.sourceId), s.filename, s.mime))}>下载原件</button></li>)}</ul><div className="mt-3 flex flex-wrap gap-2"><button className={actionClass} onClick={() => { setSelectedBatchId(b.batchId); setTab('AI 整理'); }}>继续 AI 整理</button>{snapshot.sources.some(s => s.batchId === b.batchId && s.parse.status === 'pending') && <button className={actionClass} disabled={busy} onClick={() => void act(async () => { await sources.parseBatch(b.batchId); await refresh(); })}>继续提取文字</button>}</div></section>)}
    </>}
    {(tab === 'AI 整理' || tab === '研究桥') && <section className="space-y-4 rounded border border-borderSoft bg-bg2 p-4"><h3 className="text-lg font-semibold">{tab === '研究桥' ? '只读研究桥' : '整理这一批资料'}</h3><p className="text-sm text-warning">自动分析执行器尚未连接</p>{tab === 'AI 整理' && <p className="text-sm text-textMuted">选择批次 → 交接研究 → 导入贡献包 → 本人审核。</p>}
      ...
      {tab === '研究桥' && snapshot && batch && <BridgeStagingPanel batch={batch} snapshot={snapshot} sourceRepository={sources} wiki={loadedWiki.data} model={wikiRead.model} />}
      {tab === 'AI 整理' && <><button className="inbox-action" onClick={() => setTab('研究桥')}>使用只读研究桥</button><details><summary className="min-h-11 cursor-pointer font-semibold">手工导出研究任务</summary><p className="text-sm text-textMuted">导出后交给 ChatGPT；导出文件含本批解析文本、原文定位和贡献包格式。</p><button className={`${actionClass} mt-2`} disabled={busy || !batch || !!wikiRead.error} onClick={() => void exportTask()}>导出本批研究任务</button></details>
      ...
      {tab === '研究桥' && snapshot && !batch && <div className="memory-compact-empty"><p>先添加资料，再选择明确授权的只读范围。</p><button className="inbox-action" onClick={() => setTab('原始资料')}>添加资料</button></div>}
    </section>}
    {tab === '待审核' && <section className="space-y-3">{reviewState.status === 'locked' || reviewState.status === 'error' ? <p role="alert" className="text-danger">知识待审暂不可读：{reviewState.message}</p> : null}<h3 className="text-lg font-semibold">审核 AI 建议</h3><p className="text-sm text-textMuted">逐项核对完整文章、变化和原文。接受前不会改变知识库。</p>{reviewState.status === 'ready' && !pending.length && <p className="rounded border border-borderSoft bg-bg2 p-4 text-sm">暂无待审核建议。完成 AI 整理并导入贡献包后，会在这里显示。</p>}
    ...
    <div hidden={tab !== '我的知识库' || !snapshot}><WikiLibrary refreshToken={epoch} {...props} active={props.active !== false && tab === '我的知识库'} onAddMaterial={props.onAddMaterial ?? (() => { setTab('原始资料'); setAddMaterial(true); })} repository={wiki} owners={owners} creatorRepository={creator} evidenceOptions={props.evidenceOptions ?? baseOwners.listEvidence(new Date().toISOString())} onMaterial={openMaterial} materialLabel={ref => 'sourceId' in ref ? snapshot?.sources.find(s => s.sourceId === ref.sourceId)?.filename ?? '已保存原文' : '已保存研究记录'} /></div>
    {addMaterial && <Modal title="添加资料" busy={busy} hasUnsavedChanges={!!paste || !!batchTitle} onClose={() => { if ((!paste && !batchTitle) || window.confirm('保留当前输入并关闭添加资料？')) setAddMaterial(false); }} error={error || undefined}>      <div className="memory-upload-form"><p className="mt-2 text-sm text-textMuted">一次选择多份 PDF、Markdown 或 TXT。原文件先保存在当前浏览器，刷新后仍可读取；不会自动上传到云端。</p><label className="mt-3 block text-sm">本批资料名称（可选）<input className={wikiInputClass} value={batchTitle} onChange={e => setBatchTitle(e.target.value)} placeholder="例如：光通信产业链·九月资料" /></label>

...
import './research-memory-workspace.css';
...
export type WikiLibraryProps = { refreshToken?: number; onBackToLibrary?: () => void; onAddMaterial?: () => void; maintenanceOpen?: boolean; selectedWikiId?: string; onSelectWiki?: (wikiId: string) => void; active?: boolean; repository?: WikiRepository; owners?: WikiOwners; creatorRepository?: CreatorViewpointRepository; evidenceOptions?: WikiEvidenceOption[]; onMaterial?: (ref: ResearchSourceRef | ResearchExtractionRef) => void; materialLabel?: (ref: ResearchSourceRef | ResearchExtractionRef) => string };
...
export function WikiLibrary(props: WikiLibraryProps) {
  ...
  useEffect(() => { if (props.maintenanceOpen) setMaintenance(true); }, [props.maintenanceOpen]);
  const [showDrafts, setShowDrafts] = useState(false);
  const [openHistory, setOpenHistory] = useState<Set<string>>(() => new Set());
  const [selectedId, setSelectedId] = useState(props.selectedWikiId ?? ''), [query, setQuery] = useState(''), [type, setType] = useState('');
  useEffect(() => { setSelectedId(props.selectedWikiId ?? ''); }, [props.selectedWikiId]);
  ...
  useEffect(() => { if (props.active === false) return; setLoaded(repository.load()); setNow(new Date().toISOString()); setVault(null); setProjection('需重新生成导出文件'); }, [repository, props.active, props.refreshToken]);
  ...
  const selected = visibleEntries.find(entry => entry.wikiId === selectedId && (showDrafts || read.model?.pages.some(page => page.entry.wikiId === entry.wikiId)));
  const detailRequested = !!selectedId || !!props.selectedWikiId;
  const backToList = () => { setSelectedId(''); props.onBackToLibrary?.(); };
  ...
  const append = (additions: WikiAdditions) => {
    ...
    if (additions.revisions?.length) { setShowDrafts(true); setSelectedId(additions.revisions[0].wikiId); props.onSelectWiki?.(additions.revisions[0].wikiId); setQuery(''); setType(''); }
  };
  ...
  return <section aria-label="知识库文章区" className="space-y-4 min-w-0">
    ...
    {!detailRequested && <>
      {data.entries.length > 0 && <div className="wiki-library-filters"><label className="text-xs text-textMuted break-words">搜索知识库<input className={`${wikiInputClass} mt-1`} value={query} onChange={event => setQuery(event.target.value)} placeholder="标题、摘要、别名、标签、正文与关联" /></label><label className="text-xs text-textMuted break-words">类型筛选<select className={`${wikiInputClass} mt-1`} value={type} onChange={event => setType(event.target.value)}><option value="">全部类型</option>{WIKI_TYPES.map(value => <option key={value} value={value}>{wikiTypeLabels[value]}</option>)}</select></label></div>}
      {entries.length > 0 && <div aria-label="文章列表" className="wiki-library-list">{entries.map(entry => { const current = read.model?.pages.find(row => row.entry.wikiId === entry.wikiId); return <button key={entry.wikiId} aria-pressed={selected?.wikiId === entry.wikiId} className="wiki-library-row" onClick={() => { setSelectedId(entry.wikiId); props.onSelectWiki?.(entry.wikiId); }}><span className="block break-words font-semibold">{current?.revision.title ?? data.revisions.find(row => row.wikiId === entry.wikiId && Date.parse(row.createdAt) <= Date.parse(cutoff))?.title ?? entry.wikiId}</span><span className="mt-1 block text-xs text-textMuted">{wikiTypeLabels[entry.type]} · {current ? '已审核' : '尚未审核'}</span></button>; })}</div>}
      {!entries.length && !read.error && <div className="wiki-library-empty" data-state={query.trim() || type ? 'FILTERED_EMPTY' : 'READY_EMPTY'}><div><h3>{query.trim() || type ? '没有匹配的文章' : data.entries.length ? '暂无已审核文章' : '还没有文章'}</h3><p>{query.trim() || type ? '调整搜索或类型筛选后重试。' : '从原始资料开始，审核后形成可追溯的知识。'}</p></div>{query.trim() || type ? <button className="inbox-action" onClick={() => { setQuery(''); setType(''); }}>清除筛选</button> : data.entries.length ? <button className="inbox-action" onClick={() => setShowDrafts(true)}>查看草稿与归档</button> : props.onAddMaterial ? <button className="inbox-action" onClick={props.onAddMaterial}>添加资料</button> : !maintenance ? <button className="inbox-action" disabled={blocked} onClick={() => setForm({})}>手工新建文章</button> : null}</div>}
    </>}
    {detailRequested && selected && <article aria-label="文章详情" className="wiki-reading-article">{selected ? <>
          <div className="flex flex-wrap justify-between gap-3"><div className="min-w-0"><h3 className="wiki-reading-title break-words">{page?.revision.title ?? last?.title ?? selected.wikiId}</h3><p className="mt-1 text-xs text-textMuted">{wikiTypeLabels[selected.type]}</p><details><summary className="text-xs text-textMuted">审计详情</summary><p className="break-all text-xs">wikiId · {selected.wikiId}</p></details></div><button className="inbox-action !whitespace-normal max-w-full" disabled={blocked || !last} onClick={() => setForm({ previous: last })}>追加修订</button></div>
          {page ? <div className="wiki-reading-content"><p className="text-sm text-cyan">当前已审核文章 · {page.origin === 'ai_draft' ? 'AI 整理，人工审核' : '用户整理'}</p><p className="text-xs text-warning">来源与结论仍需持续核验，审核不代表事实已证实。</p><details><summary className="text-xs text-textMuted">审计详情</summary><p className="text-xs break-all">{page.revision.revisionId} · {page.revision.asOf} · {page.completeness} · {page.origin} · {page.uncertainty.join(' / ')}</p></details><p className="wiki-reading-summary">{page.revision.summary}</p>{page.uncertainty.length > 0 && <p className="wiki-reading-limitation break-words text-warning" aria-label="文章重要限制">重要限制：{page.uncertainty.join('；')}</p>}<KnowledgeDocument text={page.revision.bodyMarkdown} />
            <section><h4 className="font-semibold">证据与来源</h4><div className="mt-2 flex flex-col items-start gap-2">{page.revision.sourceRefs.map(ref => <button key={ref.sourceId} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => openMaterial(ref, page.revision.asOf)}>原始资料 · {props.materialLabel?.(ref) ?? '已保存原文'}</button>)}{page.revision.extractionRefs.map(ref => <button key={ref.extractionId} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => openMaterial(ref, page.revision.asOf)}>提取内容 · {props.materialLabel?.(ref) ?? '已保存研究记录'}</button>)}{page.revision.evidenceRefs.map((ref, index) => <button key={JSON.stringify(ref)} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => act(() => setAudit(owners.evidence(ref, page.revision.asOf).audit))}>证据 · 查看依据 {index + 1}<span className="sr-only">（{ref.objectId}）</span></button>)}</div></section>
            <section><h4 className="font-semibold">关联文章与引用关系</h4><div className="mt-2 flex flex-wrap gap-2">{page.revision.wikiRefs.map(ref => <button className="inbox-action !whitespace-normal max-w-full" key={`out-${ref.wikiId}`} onClick={() => { setQuery(''); setType(''); setSelectedId(ref.wikiId); props.onSelectWiki?.(ref.wikiId); }}>关联 · {read.model?.pages.find(row => row.entry.wikiId === ref.wikiId)?.revision.title}</button>)}{page.backlinks.map(id => <button className="inbox-action !whitespace-normal max-w-full" key={`in-${id}`} onClick={() => { setQuery(''); setType(''); setSelectedId(id); props.onSelectWiki?.(id); }}>反向引用 · {read.model?.pages.find(row => row.entry.wikiId === id)?.revision.title}</button>)}</div>{page.orphan && <p className="mt-2 text-xs text-textMuted">暂无关联文章。支持材料仍可单独反查。</p>}</section>
          </div> : <p className="mt-4 text-sm text-warning">尚无已审核文章。草稿、拒绝或归档记录保留在下方历史。</p>}
          <section className="mt-5 border-t border-borderSoft pt-4"><h4 className="font-semibold">时间与版本演化</h4>{history.map(revision => { const status = wikiRevisionStatus(data, revision.revisionId, cutoff); return <details key={revision.revisionId} onToggle={event => { if (event.target !== event.currentTarget) return; const open = event.currentTarget.open; setOpenHistory(previous => { const next = new Set(previous); if (open) next.add(revision.revisionId); else next.delete(revision.revisionId); return next; }); }} className="mt-2 rounded border border-borderSoft p-3"><summary className="min-h-11 cursor-pointer break-words text-sm">{revision.title} · {{ draft: '待审核', reviewed: '已审核', rejected: '已拒绝', archived: '已归档' }[status]} · {revision.authorType === 'ai' ? 'AI 整理' : '用户整理'} · {revision.createdAt}</summary><p className="mt-2 text-xs text-textMuted break-all">版本标识：{revision.revisionId}</p><p className="mt-2 text-sm">{revision.revisionReason}</p><p className="mt-2 text-xs text-textMuted break-words">截至 {revision.asOf} · {revision.summary}</p><div className="mt-2">{openHistory.has(revision.revisionId) && <KnowledgeDocument text={revision.bodyMarkdown} />}</div><div className="mt-2 flex flex-wrap gap-2">{revision.sourceRefs.map(ref => <button key={ref.sourceId} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => openMaterial(ref, revision.asOf)}>来源 · {props.materialLabel?.(ref) ?? '已保存原文'}</button>)}{revision.extractionRefs.map(ref => <button key={ref.extractionId} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => openMaterial(ref, revision.asOf)}>提取 · {props.materialLabel?.(ref) ?? '已保存研究记录'}</button>)}{revision.evidenceRefs.map((ref, index) => <button key={JSON.stringify(ref)} className="inbox-action !whitespace-normal max-w-full break-all text-left" onClick={() => act(() => setAudit(owners.evidence(ref, revision.asOf).audit))}>证据 · 查看依据 {index + 1}<span className="sr-only">（{ref.objectId}）</span></button>)}</div><div className="mt-2 flex flex-wrap gap-2">{status === 'draft' && (['reviewed', 'rejected'] as const).map(decision => <button key={decision} className="inbox-action !whitespace-normal max-w-full" disabled={blocked} onClick={() => { setReview({ revision, decision }); setReviewNote(''); }}>{decision === 'reviewed' ? '审核此修订' : '拒绝此修订'}</button>)}{status === 'reviewed' && <button className="inbox-action !whitespace-normal max-w-full" disabled={blocked} onClick={() => { setReview({ revision, decision: 'archived' }); setReviewNote(''); }}>归档此修订</button>}</div>{data.reviews.filter(row => row.revisionId === revision.revisionId && Date.parse(row.createdAt) <= Date.parse(cutoff)).map(row => <p key={row.reviewId} className="mt-2 break-all text-xs text-textMuted">{{ reviewed: '已审核', rejected: '已拒绝', archived: '已归档' }[row.decision]} · {row.createdAt} · {row.note}</p>)}</details>; })}</section>

...
import { chartNumericValue, financialChartAudit } from "../../services/chartAudit";
import { safeEvidenceUrl } from "../../utils/evidenceUrl";

type Selection = { period: "singleQuarter" | "cumulative"; scope: string };
export function CompanyFinancialHistory({ stock, detail, selection, onSelectionChange }: { stock: Stock; detail: AShareFinancialData | null; selection?: Selection; onSelectionChange?: (selection: Selection) => void }) {
  ...
  const reports = (owner?.reports ?? []).filter(report => report.stockCode === owner?.stockCode && report.market === owner?.market).sort((a, b) => a.reportPeriod.localeCompare(b.reportPeriod) || a.sourceIdentifier.localeCompare(b.sourceIdentifier) || JSON.stringify(a).localeCompare(JSON.stringify(b)));
  const scopes = [...new Set(reports.map((report) => report.statementScope))].sort();
  ...
  const rows = reports.filter((report) => report.statementScope === currentScope).map((report) => ({
    ...
    revenue: report.status === "conflicted" ? null : chartNumericValue(report[period]?.operatingRevenue),
    profit: report.status === "conflicted" ? null : chartNumericValue(report[period]?.netProfitAttributableToParent),
    cashFlow: report.status === "conflicted" ? null : chartNumericValue(report[period]?.netOperatingCashFlow),
  })).sort((left, right) => left.period.localeCompare(right.period));
  return <div className="space-y-4">
    {!reports.length ? <ChartAuditPanel audit={financialChartAudit(stock, owner, reports, period, currentScope)} /> : null}
    {!reports.length ? <p className="rounded-lg border border-borderSoft bg-bg2 p-4 text-sm text-textMuted">完整历史尚不可用；当前财务快照和加载状态见上方。历史趋势需要连续报告数据。</p> : <>
      ...
      <ChartPanel title="分期经营读数" description={`${period === "singleQuarter" ? "单季度" : "年初至报告期累计"} · ${scopeName(currentScope)} · 人民币 / 元；沿用报告原有口径，缺值保留。`}
        audit={<ChartAuditPanel audit={financialChartAudit(stock, owner, reports, period, currentScope)} />}
        ...
        <div className="h-72" role="img" aria-label="分期经营柱形图，原始数值可展开数据表"><ResponsiveContainer width="100%" height="100%"><BarChart data={rows} margin={{ top: 10, right: 12, left: 0, bottom: 5 }}><CartesianGrid stroke={chartColors.grid} strokeDasharray="3 3" /><XAxis dataKey="period" tick={chartTickStyle} minTickGap={28} /><YAxis tick={chartTickStyle} width={70} tickFormatter={formatAxisNumber} /><Tooltip contentStyle={chartTooltipStyle} formatter={(value: number) => formatFinancialAmount(value)} /><Legend /><Bar dataKey="revenue" name="营业收入" fill={chartColors.primary} isAnimationActive={false} /><Bar dataKey="profit" name="归母净利润" fill={chartColors.secondary} isAnimationActive={false} /><Bar dataKey="cashFlow" name="经营现金流（描边）" fill={chartColors.info} stroke="var(--ui-text)" strokeWidth={1.5} strokeDasharray="3 2" isAnimationActive={false} /></BarChart></ResponsiveContainer></div>
      </ChartPanel>
      <details className="rounded-lg border border-control bg-bg2 p-4"><summary className="cursor-pointer text-sm font-semibold text-accent">三表完整历史与原始报告（{reports.length} 期）</summary><div className="mt-4 space-y-3">{reports.map((report, index) => <details key={`${report.reportPeriod}-${index}`} className="min-w-0 rounded border border-borderSoft p-3"><summary className="cursor-pointer break-words text-sm text-textStrong">{report.reportPeriod} · {scopeName(report.statementScope)} · {report.status} · 人民币 / 元</summary><p className="mt-2 break-words text-xs text-textMuted">发布：{report.announcementDate ?? "未提供"} · 来源：{report.provider} · 抓取：{report.fetchedAt} · {report.isDerived ? report.derivationMethod ?? "使用已推导字段" : "报告字段"}</p>{safeEvidenceUrl(report.sourceUrl) ? <a href={safeEvidenceUrl(report.sourceUrl)!} target="_blank" rel="noreferrer" className="mt-2 inline-block break-all text-xs text-accent">查看报告来源</a> : null}<ReportTables report={report} /><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-all text-xs leading-5 text-textMuted">{JSON.stringify(report, null, 2)}</pre></details>)}</div></details>
    </>}
  </div>;
}

...
describe("StockDetailDrawer watchlist linkage", () => {
  it("renders the join-watchlist action for a company without an active item", () => {
    const html = renderToStaticMarkup(<StockDetailDrawer activeTab="evidence" stock={stocks[0]} stocks={stocks} industries={industries} onClose={() => undefined} />);
  ...
  it("renders current status and review entry instead of a duplicate add action", () => {
    ...
    const html = renderToStaticMarkup(<StockDetailDrawer activeTab="evidence" stock={stocks[0]} stocks={stocks} industries={industries} watchItems={[item]} onClose={() => undefined} />);

    ...
    <article className="rounded-lg border border-borderSoft bg-bg2/65 p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          {event.expectation ? <p className="mt-1 text-xs text-textMuted">原业务形成时间：{event.expectation.originalBusinessTime ?? "缺失"}（{event.expectation.businessTimePrecision ?? "date"}）{event.expectation.correctionRecordedAt ? ` · 纠正记录时间：${event.expectation.correctionRecordedAt}` : ""}{event.expectation.businessOrderStatus === "uncertain" ? " · 业务顺序不确定" : event.expectation.businessOrderStatus === "equal" ? " · 精确时刻相同，不代表先后" : ""}</p> : null}

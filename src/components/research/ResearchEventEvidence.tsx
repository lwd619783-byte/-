    ...
    <article className="rounded-lg border border-borderSoft bg-bg2/65 p-4">
      <div className="flex min-w-0 flex-col gap-3">
        <div className="min-w-0">
          ...
          {event.expectation ? <p className="mt-1 break-words text-xs leading-5 text-textMuted">原记录时间：{event.expectation.originalBusinessTime ?? "缺失"}（{statusDisplayLabel(event.expectation.businessTimePrecision ?? "unknown")}） · 当前有效时间：{event.expectation.effectiveBusinessTime ?? "缺失"}（{statusDisplayLabel(event.expectation.effectiveBusinessTimePrecision ?? "unknown")}）{event.expectation.temporalCorrectionApplied ? ` · 时间字段已纠正：${event.expectation.correctedTemporalFields?.join("、") || "待核验"}` : ""}{event.expectation.correctionRecordedAt ? ` · 纠正记录时间：${event.expectation.correctionRecordedAt}` : ""}{event.expectation.businessOrderStatus === "uncertain" ? " · 业务顺序不确定" : event.expectation.businessOrderStatus === "equal" ? " · 精确时刻相同，不代表先后" : ""}</p> : null}
          {event.expectation?.ingestionMethod === "provider" ? <p className="mt-1 text-xs text-cyan">来源记录 · 数据版本 {event.expectation.providerVersion ?? "-"} · 更新 {event.expectation.providerGeneratedAt ?? "-"} · 公告标识 {event.expectation.sourceAnnouncementId ?? "-"} · 公司内部形成时间未提供；公开可得性须独立核验</p> : null}
      ...
      {event.expectation ? <section className="mt-4 text-sm leading-6" aria-label="预期原始快照数值"><h4 className="font-semibold">预期证据原始数值</h4>{expectationSnapshot ? <><p>{expectationSnapshot.metric} · {expectationSnapshot.reportPeriod} · {expectationSnapshot.periodScope}</p><p>{expectationSnapshot.estimateShape === "point" ? expectationSnapshot.value ?? "缺失" : `${expectationSnapshot.lowerBound ?? "缺失"} 至 ${expectationSnapshot.upperBound ?? "缺失"}`} · {expectationSnapshot.currency} · {expectationSnapshot.unit}</p><p className="text-xs text-textMuted">会计口径：{expectationSnapshot.accountingBasis} · 原始快照：{expectationSnapshot.id}</p></> : <p className="text-warning">当前未提供可唯一匹配的原始快照，数值单位与完整口径未证明。请进入公司预期证据核对。</p>}</section> : null}
...
function EvidenceProvenance({ event }: { event: ResearchEvent }) {
  ...
  return <section className="mt-4 space-y-3" aria-label="证据来源与时间明细">
    ...
    <dl className="grid gap-3 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label} className="min-w-0"><dt className="text-xs text-textMuted">{label}</dt><dd className="mt-1 break-all text-sm">{value || "未提供"}</dd></div>)}</dl>
    ...
    <div className="rounded-md border border-warning/40 bg-warning/10 p-3 text-xs leading-6 text-warning" aria-label="证据证明边界">
      <p>严格 PIT：未证明 · 正式 releaseAvailableAt：未提供</p>
      <p>生产 / 数据准入：未证明（unknown） · 正式 revision 连续性：未证明</p>
      <p>Evidence Graph 闭合性：未证明 · 保留原文与定位证据：未提供</p>

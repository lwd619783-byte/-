...
export function ResearchWorkbench({ stocks, watchItems = [], tasks = [], events = [], industryEvents, expectationSnapshots, now, timeZone = "Asia/Shanghai", inboxSourceNotice, dataState = "ready", dataMessage, observationState = dataState, observationMessage, onNavigate, onOpenStock, onStartReview, onOpenEvent, onOpenTasks, onOpenSources, onOpenResearch }: ResearchWorkbenchProps) {
  ...
  const showSide = isReadableResearchState(observationState) && isReadableResearchState(dataState) && pending.length > 0;
  return <section className="ui-v2-workbench" aria-label="工作台">
    ...
    <div className={`ui-v21-home-columns ${showSide ? "has-tasks" : ""}`} data-columns={showSide ? "2" : "1"}>
      <section aria-labelledby="workbench-observations-title" className="ui-v21-observations">
        ...
        {!isReadableResearchState(observationState) ? <p role={observationState === "loading" ? "status" : "alert"} className="ui-v21-quiet-strip">{observationMessage ?? dataMessage ?? ({ loading: "正在载入观察记录，数量尚未确定。", error: "观察记录读取失败，不能判断是否为空。", locked: "观察记录已锁定，请先处理存储或权限问题。" })[observationState]}</p> : watched.length ? <ul className="ui-v21-watch-list">{watched.slice(0, 4).map(item => {
    ...
    {!showSide && isReadableResearchState(dataState) ? <button type="button" className="ui-v21-text-button ui-v21-task-link" onClick={onOpenTasks}>查看待办分类 <ArrowRight size={16} aria-hidden="true" /></button> : null}
  </section>;
}
...
export function TaskWorkspace({ stocks, watchItems = [], tasks = [], events = [], expectationSnapshots, now, timeZone = "Asia/Shanghai", inboxSourceNotice, dataState = "ready", dataMessage, observationState = dataState, observationMessage, onOpenStock, onStartReview, onOpenEvent, onOpenReview, activeQueue, onQueueChange, knowledgeState, knowledgeReview }: TaskWorkspaceProps) {
  const displayNow = useDisplayNow(now);
  ...
  const count = (value: TaskQueue) => {
    ...
    const state = value === "review" && !isReadableResearchState(observationState) ? observationState : dataState;
    return isReadableResearchState(state) ? `${state === "partial" ? "当前可读 " : ""}${value === "review" ? reviewRows.reduce((sum, row) => sum + row.tasks.length, 0) : verificationEvents.length}` : state === "loading" ? "载入中" : state === "locked" ? "已锁定" : "读取失败";
  };
  ...
  return <section className="ui-v2-task-workspace" aria-label="任务工作区">
    ...
    {queue === "knowledge" ? knowledgeReview ? <div role="tabpanel" id="task-panel-knowledge" aria-labelledby="task-tab-knowledge">{knowledgeReview}</div> : null : <div role="tabpanel" id={`task-panel-${queue}`} aria-labelledby={`task-tab-${queue}`}>
      <ResearchInbox key={queue} mode={queue} stocks={stocks} watchItems={watchItems} tasks={queue === "review" ? tasks : []} events={queue === "verification" ? verificationEvents : events} expectationSnapshots={expectationSnapshots} now={displayNow} timeZone={timeZone} sourceNotice={inboxSourceNotice} dataState={queue === "review" && !isReadableResearchState(observationState) ? observationState : dataState} dataMessage={queue === "review" && !isReadableResearchState(observationState) ? observationMessage ?? dataMessage : dataMessage} onOpenStock={onOpenStock} onStartReview={onStartReview} onOpenEvent={onOpenEvent} />
    </div>}
  </section>;
}

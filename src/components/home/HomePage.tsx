...
export function HomePage({ dataMode, modeLabel, updatedAt, sourceNote, coverageSummary, stats, focusStocks, quoteStocks, activeWatchCount, onNavigate, onOpenStock, now, watchItems=[], tasks=[], events=[], onStartReview, onOpenEvent }: HomePageProps) {
  ...
  return <section className="home-workbench space-y-5" aria-label="首页研究工作台">
    ...
    <div className="home-research-grid">
      <DashboardCard className="home-priorities p-4">
        <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-base font-semibold">今日优先事项</h2><button type="button" onClick={()=>onNavigate("观察清单")} className="min-h-11 text-sm text-accent">查看全部待办{pending.length ? `（${pending.length}）` : ""}</button></div>
        {pending.length ? <div className="home-priority-list">{pending.slice(0,3).map(task=>{const item=watchItems.find(item=>item.id===task.watchItemId);const stock=quoteStocks.find(stock=>stock.id===item?.stockId);return <article key={task.id} className="min-w-0 border-t border-borderSoft pt-3"><div className="flex flex-wrap items-start justify-between gap-2"><h3 className="font-medium">{stock?.name ?? "研究对象待核验"}</h3><span className="text-xs text-warning">{task.dueAt ?? "待核验"}</span></div><p className="mt-1 text-sm">{task.title}</p><p className="mt-1 text-xs leading-5 text-textMuted">{task.description}</p>{item ? <button type="button" onClick={()=>onStartReview?.(item)} className="min-h-11 text-sm text-accent">开始复盘</button>:null}</article>;})}</div> : <p className="text-sm leading-6 text-textMuted"><span>暂无待处理研究任务。</span> 添加观察项后，按约定复盘时间和已有研究事件查看提醒。</p>}
      </DashboardCard>
      <div className="home-price min-w-0 space-y-3"><label className="flex flex-wrap items-center justify-between gap-2 text-xs text-textMuted">研究对象价格脉络<select aria-label="价格脉络研究对象" value={selected?.id ?? ""} onChange={event=>setPriceId(event.target.value)}>{!quoteStocks.length ? <option value="">暂无研究对象</option>:null}{quoteStocks.map(stock=><option key={stock.id} value={stock.id}>{stock.name} · {stock.code}</option>)}</select></label>{selected ? <StockPriceHistoryChart stock={selected} compact/>:<DashboardCard className="p-6 text-textMuted">研究池暂无对象，价格历史不可用。</DashboardCard>}</div>
      <DashboardCard className="home-events flex flex-col p-5"><h2 className="text-lg font-semibold">最近研究事件</h2><p className="mt-1 text-xs text-textMuted">现有研究事件，保留原始来源与时间</p><div className="my-3 space-y-3">{events.slice(0,3).map(event=><article key={event.id} className="border-l-2 border-accent/60 pl-3"><button type="button" className="min-h-11 text-left text-sm font-medium hover:text-accent" onClick={()=>onOpenEvent ? onOpenEvent(event) : onNavigate("验证中心")}>{event.title}</button><p className="mt-1 text-xs leading-6 text-textMuted">{event.stockName} · {event.eventDate ?? event.publishedAt ?? "时间未提供"}</p><p className="text-xs text-textMuted">{event.sourceName} · {statusDisplayLabel(event.verificationStatus)}</p></article>)}{!events.length ? <p className="py-6 text-sm text-textMuted">当前没有研究事件。</p>:null}</div><button type="button" onClick={()=>onNavigate("验证中心")} className="mt-auto min-h-11 self-start text-sm text-accent">查看全部研究事件</button></DashboardCard>
      ...
      <DashboardCard className="home-data min-w-0 p-5"><h2 className="text-lg font-semibold">数据状态</h2><p className="mt-2 text-sm text-textMuted">{coverageSummary}</p><p className="mt-2 text-xs text-textMuted">A 股行情覆盖：{stats.quoteStatusRealCovered} / {stats.quoteCoverageTotal}（质量状态 real 且有价格）</p>
    <details className="mt-3 text-xs text-textMuted"><summary>行情覆盖与来源明细</summary><div className="mt-3 space-y-2"><p>当前模式：{dataModeDisplayLabel(modeLabel)}。{localizeDataSourceNote(sourceNote)}</p><p>{describeDataTime(dataMode==="mock" ? undefined : updatedAt,"package_updated",displayNow).text}</p><p>A 股行情覆盖（质量状态 real 且有价格）：{stats.quoteStatusRealCovered} / {stats.quoteCoverageTotal}</p><QuoteTrustSummary stocks={quoteStocks} now={displayNow}/><p>{coverageSummary}</p>{selected ? <QuoteTrust quote={selected.quote} now={displayNow}/>:null}</div></details></DashboardCard>
    </div>

...
export function ResearchInbox({ events, tasks, watchItems, stocks, now, timeZone, sourceNotice, expectationSnapshots, onOpenStock, onOpenEvent, onStartReview }: ResearchInboxProps) {
  const rows = useMemo(() => buildResearchInbox({ events, tasks, watchItems, now, timeZone }), [events, tasks, watchItems, now, timeZone]);
  ...
  return <DashboardCard className="home-priorities p-4" >
    <section aria-label="Research Inbox">
      ...
      <details className="mt-3 text-xs leading-6 text-textMuted"><summary className="cursor-pointer">排序与数据范围</summary><p>逾期任务 → 今日任务 → 其他待处理任务 → 待复盘事件 → 近期变化；左栏仅列已有待处理任务；右栏列未合并事件，默认近 30 天。各组按原始严重程度 / 影响程度、任务日期升序或事件日期降序、稳定标识排序。同一观察项合并展示所有待处理任务。已关联任务的事件不重复列出，包括已确认、忽略和稍后处理的任务。</p><p>日期沿用原事件记录；财务摘要可能使用报告期，更新 / 检测时间不作为新事件日期。此处是当前已载入数据的研究提醒，不是实时新闻或投资评分。严格 PIT 与准入须在证据中单独核验。</p></details>

...
const fresh = async () => {
  ...
  await page.goto(`${origin}/#/memory`); await page.getByRole('region', { name: '研究记忆工作区' }).waitFor(); await page.getByRole('navigation', { name: '研究记忆视图' }).getByRole('button', { name: '我的知识库', exact: true }).click({ noWaitAfter: true }); await page.getByText('更多操作', { exact: true }).click({ noWaitAfter: true });
...
const navigate = async (page, name) => { const button = page.getByRole('navigation', { name: '研究记忆视图' }).getByRole('button', { name, exact: true }); await button.click({ noWaitAfter: true }); await button.and(page.locator('[aria-pressed=true]')).waitFor(); };
...
const reviewDraft = async (page, title, decision = 'reviewed') => {
  ...
  await history.locator('summary').click({ noWaitAfter: true }); await history.getByRole('button', { name: decision === 'reviewed' ? '审核此修订' : '拒绝此修订', exact: true }).click({ noWaitAfter: true });
...
try {
  ...
  await details(page).getByRole('button', { name: '提取内容 · 已保存研究记录', exact: true }).click({ noWaitAfter: true }); check(await page.getByRole('dialog').getByText('合成测试摘录，不是任何真实博主观点。').count() === 1, 'Wiki to Extraction to Raw Source'); await page.getByRole('dialog').getByRole('button', { name: '关闭', exact: true }).click({ noWaitAfter: true });
  await details(page).getByRole('button', { name: /^证据 ·/ }).first().click({ noWaitAfter: true }); check(await page.getByRole('dialog', { name: '证据核对', exact: true }).count() === 1, 'existing Evidence Drawer reused'); await shot(page, 'evidence'); await page.getByRole('dialog').getByRole('button', { name: '关闭', exact: true }).click({ noWaitAfter: true });
  ...
  await page.setViewportSize({ width: 1536, height: 960 }); await w.getByText('更多操作', { exact: true }).click({ noWaitAfter: true });

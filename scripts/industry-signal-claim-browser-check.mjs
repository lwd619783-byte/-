/** Slice 6 derived candidates plus Slice 5 regression; actual retained owners, isolated Edge context, no user profile or live acquisition. */
...
try {
  ...
  for (const theme of ['light']) for (const width of [320, 390, 1536]) {
    ...
    for (const industry of ['oil-shipping', 'robotics']) {
      ...
      const signalPanel = page.getByRole('region', { name: '派生信号与景气判断资格', exact: true });
      await signalPanel.locator('article').first().waitFor();
      const candidateText = await signalPanel.innerText();
      check(!internal.test(candidateText), `Chinese candidate default ${name}`);
      check(candidateText.includes('景气判断资格：暂不具备条件') && candidateText.includes('当前暂不生成正式行业景气结论'), `abstention ${name}`);
      for (const reason of ['数据或生产尚未准入', '历史时点可得性未证明', '公开可得时间未知', '官方修订连续性未知', '必需研究维度缺失']) check(candidateText.includes(reason), `blocker ${reason} ${name}`);
      check(await signalPanel.locator('article').count() === (industry === 'oil-shipping' ? 4 : 1), `reviewed signal roster ${name}`);
      ...
      if (['light-1536', 'light-390', 'light-320'].includes(`${theme}-${width}`)) {
        ...
        if (width < 600) {
          ...
          await page.screenshot({ path: path.join(output, `signals-${name}.png`) });
        } else await signalPanel.screenshot({ path: path.join(output, `signals-${name}.png`) });
        report.screenshots.push(`signals-${name}.png`);
      }
      for (const row of await signalPanel.locator('article').all()) {
        const heading = await row.locator('h3').innerText();
        const trigger = row.getByRole('button', { name: '查看完整证据链' });
        await trigger.click(); const drawer = page.getByRole('dialog'); await drawer.waitFor();
        check((await drawer.innerText()).includes('候选结论 → 派生信号'), `candidate signal trace ${heading} ${name}`);
        check(!internal.test(await drawer.innerText()), `Chinese candidate evidence ${heading} ${name}`);
        check(await drawer.getByRole('link').count() === 2, `exact two operand sources ${heading} ${name}`);
        const details = drawer.locator('[data-advanced-audit]'); check(!await details.evaluate(e => e.open), `collapsed graph audit ${heading} ${name}`);
        await details.locator('summary').click(); const text = await details.innerText();
        for (const field of ['evidence-graph.v1', 'formulaRef', 'inputManifestRef', 'input_to', 'supports', 'source_material', 'provider_fact', 'derived_result', 'ai_draft', 'NOT_ADMITTED', 'releaseAvailableAt']) check(text.includes(field), `F2 ${field} ${heading} ${name}`);
        const owner = [...owners.values()].find(o => o.definition.canonicalName === heading);
        const inputs = owner.observations.filter(o => o.basis === (industry === 'robotics' ? 'monthly' : 'week_ending')).slice(-2);
        for (const observation of inputs) {
          check(text.includes(observation.id) && text.includes(observation.provenance.rawSha256) && text.includes(observation.provenance.locator), `exact observation/raw/locator ${observation.valueDate} ${heading} ${name}`);
        }
        await fits(`candidate drawer ${name}`);
        await page.keyboard.press('Shift+Tab'); check(await drawer.evaluate(e => e.contains(document.activeElement)), `candidate focus trapped ${heading} ${name}`);
        await page.keyboard.press('Escape'); check(await trigger.evaluate(e => e === document.activeElement), `candidate focus restored ${heading} ${name}`);
      }
      await fits(`signals ${name}`);

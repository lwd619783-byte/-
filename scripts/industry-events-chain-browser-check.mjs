...
try {
  ...
  for (const theme of ['neon', 'pro', 'light']) for (const width of [1536, 390, 320]) {
    ...
    const robotSummary = diagram.locator('[data-chain-stage="下游"] [data-chain-group="robot-oem"] .chain-segment-detail > summary');
    await robotSummary.focus();
    await page.keyboard.press('Enter');
    check(await unitree.isVisible(), `segment keyboard dropdown reveals companies ${name}`);
    ...
    await page.keyboard.press('Escape');
    await robotSummary.click();
    check(!await unitree.isVisible(), `segment dropdown closes ${name}`);
  ...
  await page.getByRole('tab', { name: '产业链', exact: true }).click();
  check(await page.locator('[data-chain-group="robot-oem"] > details > summary').isVisible(), 'Inbox reaches compact chain');

...
const output = path.resolve(process.env.UI_REVIEW_OUTPUT || 'data-cache/stage-4-2-slice-3/segment-architecture-browser');
await fs.mkdir(output, { recursive: true });
const report = { base: '086521d6bd305ea73cb5d4a426b9d4138e4a824b', auditedInput: 'a1a4be2a6d126866eac00c1fc693ac6608728d28', scope: 'Slice 3 segment-node architecture and cohort 57 remediation', generatedAt: new Date().toISOString(), sourceSha256: {}, checks: [], errors: [], screenshots: [], screenshotNote: 'Element captures hide the fixed mobile navigation only during screenshot; layout and navigation assertions use unmodified UI.' };
...
try {
  ...
  for (const theme of ['neon', 'pro', 'light']) for (const width of [1536, 390, 320]) {
    ...
    const unitreeChip = diagram.locator('[data-chain-stage="下游"] [data-chain-group="robot-oem"] [data-chain-company-chip="unitree"]');
    const unitree = diagram.locator('[data-chain-stage="下游"] [data-chain-company="unitree"]');
    check(await unitreeChip.isVisible(), `Unitree secondary chip directly visible ${name}`);
    check((await unitreeChip.innerText()).includes('688836.SH') && (await unitreeChip.innerText()).includes('科创板'), `Unitree exact listed identity ${name}`);
    check(!(await unitreeChip.innerText()).includes('未上市'), `no stale Unitree private state ${name}`);
    ...
    check(await diagram.locator('.chain-segment header').evaluateAll(nodes => nodes.every(n => parseFloat(getComputedStyle(n.querySelector('h4')).fontSize) >= 16)), `readable segment labels ${name}`);
    ...
    await diagram.locator('[data-chain-stage="上游"] [data-chain-group="motor-drive-control"] .chain-segment-detail > summary').click();
    await diagram.locator('[data-chain-stage="下游"] [data-chain-group="robot-oem"] .chain-segment-detail > summary').click();
    ...
    if (width === 1536) {
      await diagram.locator('[data-chain-stage="下游"] [data-chain-group="robot-oem"]').screenshot({ path:path.join(output, `segment-expanded-${name}.png`) });
      report.screenshots.push(`segment-expanded-${name}.png`);
    }
    ...
    await page.keyboard.press('Escape');
    check(await diagram.locator('.chain-company:visible').evaluateAll(nodes => nodes.every(n => { const r=n.getBoundingClientRect(); return r.left >= 0 && r.right <= innerWidth; })), `secondary company details inside viewport ${name}`);
  ...
  await page.getByRole('tab', { name: '产业链', exact: true }).click();
  check(await page.locator('[data-chain-company-chip="unitree"]').isVisible(), 'Inbox reaches chain and Unitree node');

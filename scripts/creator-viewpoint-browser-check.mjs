...
const output = path.resolve(process.env.UI_REVIEW_OUTPUT || 'data-cache/stage-4-2-5/browser');
...
try {
  ...
  await page.evaluate(async key => {
    ...
    const add = (id, day, overrides, reviewDay) => {
      data.observations.push({ ...base, id, recordedAt: fixtureTime(day), ...overrides });
  ...
  for (const theme of ['neon', 'pro', 'light']) for (const width of [1536, 1280, 390, 320]) {
    ...
    check(await drawer.getByRole('link', { name: '打开原始来源', exact: true }).getAttribute('href') === 'https://example.com/post/1', `drawer exact source link ${label}`);
  ...
  await corrupt.evaluate(key => localStorage.setItem(key, '{broken-history'), storageKey);
  ...
  check(await corrupt.evaluate(key => localStorage.getItem(key), storageKey) === '{broken-history', 'corruption keeps original raw storage bytes');

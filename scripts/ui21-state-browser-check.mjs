...
const report = { runtimeSha: process.env.UI_REVIEW_RUNTIME_SHA || null, deploymentId: null, origin, testedAt: new Date().toISOString(), inputType: 'isolated full production App with synthetic form entries and storage faults', checks: [], errors: [], console: [], screenshots: [], states: [], storage: [], limits: ['No personal browser profile or formal knowledge approval.', 'Negative storage fixtures and setItem faults exist only in disposable contexts.', 'Ordinary rejection must not become global owner lock; genuine persistence faults must remain visible.'] };
const sourceFiles = ['src/App.tsx', 'src/components/home/ResearchWorkbench.tsx', 'src/components/home/ResearchInbox.tsx', 'src/services/watchlistStore.ts', 'src/services/watchlistRepository.ts', 'src/services/earningsExpectationStore.ts', 'src/services/earningsExpectationRepository.ts'];
...
async function newPage() {
 ...
 page.on('pageerror', e => report.errors.push({ type: 'pageerror', message: e.message })); page.on('console', message => { if (['error', 'warning'].includes(message.type())) report.console.push({ type: message.type(), text: message.text().slice(0, 1500) }); });
...
async function inspectRoutes(page, prefix, shouldLock) {
 for (const hash of ['#/home', '#/research', '#/tasks?view=replay', '#/tasks?view=verify']) {
  await route(page, hash); const main = page.locator('main'); const text = await main.innerText();
  ...
  report.states.push({ prefix, hash, alerts, lock, rows, text: text.slice(0, 16000) });
  ...
  if (shouldLock && (hash === '#/research' || hash === '#/tasks?view=verify')) check(rows > 0, `${prefix} ${hash} healthy provider events remain readable despite owner fault`);
...
await run('A duplicate watch', async () => {
 ...
 check(await raw(mainPage, WATCH) === before, 'A rejected duplicate preserves exact owner bytes'); await d.getByRole('button', { name: '取消', exact: true }).click(); await d.waitFor({ state: 'hidden' }); await inspectRoutes(mainPage, 'A', false);
...
await run('B invalid expectation range', async () => {
 if (!mainPage || !stockId) throw Error('A setup unavailable'); await mainPage.reload(); await mainPage.locator('main').waitFor(); await mainPage.getByLabel('数据模式', { exact: true }).selectOption('mock'); const before = await raw(mainPage, EXPECT), watchBefore = await raw(mainPage, WATCH);
...
if (process.env.UI21_AB_ONLY !== '1') {
 for (const [owner, key] of [['watch', WATCH], ['expectation', EXPECT]]) for (const variant of ['corrupt', 'future']) await run(`C ${owner} ${variant}`, async () => {
  const p = await newPage(), value = variant === 'corrupt' ? '{synthetic invalid json' : JSON.stringify({ schemaVersion: 999, synthetic: true }); await p.evaluate(({ key, value }) => localStorage.setItem(key, value), { key, value }); await p.reload(); await p.locator('main').waitFor(); await p.getByLabel('数据模式', { exact: true }).selectOption('mock'); await inspectRoutes(p, `C-${owner}-${variant}`, true); check(await raw(p, key) === value, `C ${owner} ${variant} original bytes retained`);
 });

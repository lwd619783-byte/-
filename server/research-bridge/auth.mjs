...
const scope = config => config.scope ?? 'research:read';
const issuer = config => config.issuer ?? config.origin;
const namespace = config => config.namespace ?? 'research-bridge';
const authPath = config => config.authPath ?? '/api/bridge';
...
export function verifyToken(token, config, kind, now = Date.now()) {
  ...
  if (value.kind !== kind || value.iss !== issuer(config) || value.aud !== config.resource || value.sub !== config.subject || !Number.isSafeInteger(value.exp) || value.exp * 1000 <= now
    || value.scope !== scope(config) || (config.scope && value.client_id !== config.clientId)) throw new Error('UNAUTHORIZED');
  return value;
}
export function authorizeParameters(params, config) {
  ...
  if (value.client_id !== config.clientId || !config.redirects.includes(value.redirect_uri) || value.response_type !== 'code' || value.code_challenge_method !== 'S256'
    || !/^[a-zA-Z0-9_-]{43}$/.test(value.code_challenge ?? '') || !value.state || value.state.length > 2000 || value.resource !== config.resource || value.scope !== scope(config)) throw new Error('INVALID_OAUTH_REQUEST');
  return value;
}
export function oauthMetadata(config) {
  return { issuer: issuer(config), authorization_endpoint: `${config.origin}${authPath(config)}/authorize`, token_endpoint: `${config.origin}${authPath(config)}/token`,
    response_types_supported: ['code'], grant_types_supported: ['authorization_code'], token_endpoint_auth_methods_supported: ['none'],
    code_challenge_methods_supported: ['S256'], scopes_supported: [scope(config)], authorization_response_iss_parameter_supported: true };
}
export function newConsent(params, config, now = Date.now()) {
  const nonce = randomBytes(24).toString('base64url');
  return { nonce, ticket: signToken({ kind: 'consent', iss: issuer(config), aud: config.resource, sub: config.subject, scope: scope(config), client_id: config.clientId, exp: Math.floor(now / 1000) + 600, nonce, params }, config) };
}
export async function issueCode(ticket, cookie, ownerSecret, config, store, now = Date.now()) {
  ...
  await store.putNew(`${namespace(config)}/oauth/consent/${createHash('sha256').update(consent.nonce).digest('hex')}.json`, { usedAt: now });
  await store.putNew(`${namespace(config)}/oauth/codes/${createHash('sha256').update(code).digest('hex')}.json`, { ...params, expiresAt: now + 120000, subject: config.subject });
  const redirect = new URL(params.redirect_uri); redirect.searchParams.set('code', code); redirect.searchParams.set('state', params.state); redirect.searchParams.set('iss', issuer(config)); return redirect.toString();
}
export async function exchangeCode(params, config, store, now = Date.now()) {
  ...
  const codeHash = createHash('sha256').update(value.code).digest('hex'), code = await store.get(`${namespace(config)}/oauth/codes/${codeHash}.json`);
  ...
  await store.putNew(`${namespace(config)}/oauth/used/${codeHash}.json`, { at: now });
  return { access_token: signToken({ kind: 'access', iss: issuer(config), aud: config.resource, sub: config.subject, scope: scope(config), client_id: config.clientId, exp: Math.floor(now / 1000) + 3600 }, config), token_type: 'Bearer', expires_in: 3600, scope: scope(config) };
}

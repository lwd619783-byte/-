...
公司指引 Provider V2 将稳定证据身份、规范化内容和纠错版本事件拆开：`providerEvidenceIdentity` 表示同一公告业务证据，`providerContentChecksum` 只描述规范化财务内容；初始 `providerSnapshotVersionId` 保留内容寻址形式，纠错版本 ID 则稳定包含 evidence identity、`providerCorrectsVersionId` 与 content checksum，因此 A→B→A 可保留三个不同版本事件。公司业务修正公告继续使用独立的 `providerBusinessRevisionPredecessorSnapshotId`。公司内部形成时间未知，记录继续使用 `formationTimeBasis=public_disclosure_proxy`、保留 `formedAt=null`，公开公告时间只作为证据可用时间。详情、全局 workflow index、模式隔离、事务写入、目录审计和验证命令见 [A股公司指引预期 Provider V2](./company-guidance-expectation-provider-v1.md)。
...
事件携带业务根快照 ID、当前纠正链终点 ID、纠正链、来源类别、来源名称、报告期、指标、预期值或区间、原始/有效业务时间、原始/有效来源发布时间、实际解释时区、时间纠正标记与字段、纠正记录时间、业务时间精度、`before/after/equal/uncertain` 关系、披露时间状态、`correctionDelta`、`businessRevisionDelta`、比较结果和来源核验状态。纠正事件明确引用直接目标并只使用该纠正快照自己的 `createdAt` 作为 `correctionRecordedAt`；业务修订明确引用上一业务根及其有效终点，二者不混用。

实际值 Comparison 与业务修订前序是两个独立维度。只要当前快照与可靠实际值满足同公司、同报告期、同期间口径、同指标、同币种/单位、来源核验和事前时间要求，就可以生成 `comparison_available`；上一业务预测是否唯一只决定能否生成 `businessRevisionDelta`、方向性修订事件、方向性 KPI 和上修/下修 ReviewTask。于是系统允许同时表达“当前预测可以与实际值比较”和“上一业务预测不唯一，无法计算上修或下修”。Comparison 存在不会抑制业务前序数据警告。

业务前序警告作为独立 `business_order` 警告族，不与来源、可用时间、披露范围、实际值缺失或审计时间警告聚合。其 payload 明确保留 `previousResolutionStatus`、排序后的 `previousCandidateIds`、`previousCandidateEffectiveSnapshotIds`、当前 `businessRootSnapshotId` / `effectiveSnapshotId`、`structuredWarningCodes`、`nonComparableReasonCodes`、当前 `availableAt` 及范围，以及 `businessOrderCandidates` 中每个候选的稳定业务根、有效终点、来源名称、形成时间与可用时间。UI 和 ReviewTask 直接读取这些结构化字段，不从中文 `summary` 反向解析候选。

## ReviewTask 联动

只对已进入观察清单的公司生成稳定、幂等、只读任务，包括新增快照、数据更正、明显业务上修/下修、实际比较可用和数据核验。数据更正任务只使用 `correctionDelta`，并以 `correctionRecordedAt` 与 `lastReviewedAt` 比较；上修/下修任务只使用业务顺序已确认的 `businessRevisionDelta`，并列出上一业务根、当前有效基准终点和当前快照。旧预测的晚录入不会误触发新业务任务，7 月 15 日新记录的纠正则能在 7 月 1 日复盘边界之后触发纠正任务。稳定任务 ID 只由观察项、规则和事件 ID 构成，不包含运行时计算时间。业务前序任务展示候选数量、候选来源、形成时间和次级审计 ID；同一连续 episode 沿用任务 ID 并继承 acknowledged/dismissed/snoozed 状态，关系解除后再次激活会获得新事件 ID 和任务 ID，状态回到 pending，不继承旧 episode 的处理状态。
...
每个警告事件携带稳定 `warningEpisodeKey`。单快照警告继续由当前业务根、排序后的结构化 warning code、纠正链内从 false 变为 true 的激活实体，以及适用时的决定性实际/披露事件 ID 构成。`availability_uncertain`、`source_verification_pending`、`source_time_unresolved` 和 `audit_time_invalid` 可以沿单快照纠正链判断；`business_order_ambiguous`、`business_order_equal`、`business_order_unresolved` 是当前业务节点与候选前序之间的关系，禁止再用“当前快照 availableAt 是否不确定”近似。

业务前序 episode 使用专用的确定性重放：先按合法 `createdAt` 和稳定快照 ID 排列同逻辑分组的 append-only 审计变更；每一步只应用当时已存在的业务根与纠正终点，重新执行 `resolveUniquePreviousBusinessNode`；找到目标关系 warning code 最近一次从不成立变为成立的快照或纠正，将其作为激活实体。业务前序 key 由 `business_order` 警告族、warning code、当前业务根、激活实体和激活时排序后的候选业务根集合构成。候选有效终点保留在 payload，但不机械进入 episode 身份，因此不改变关系状态的无关数值/备注纠正不会重置任务。中文文案、候选输入顺序、显示时区、页面刷新和 LocalStorage 重载也不参与身份：
...
- 决定性实际或披露事件只影响自己的警告族；其变化不得重置持续存在的业务前序 episode，业务前序候选变化也不得重置独立的来源核验任务；

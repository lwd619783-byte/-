...
原业务时间与纠正时间职责分离：业务根节点的 `formedAt`（否则 `asOfDate`）决定业务时间线和后续修订位置；纠正快照自身不可变的 `createdAt` 是 `correctionRecordedAt`，决定纠正事件的 `eventDate`、`publishedAt` 及其是否晚于 `lastReviewedAt`。纠正事件 payload 同时保留根快照、当前终点、完整纠正链、原业务时间、纠正记录时间、纠正原因和变化字段，绝不把纠正操作包装成新的业务预测。
...
- `sourcePublishedAt` 输入契约：`YYYY-MM-DD` 保持日期语义；带 `Z` 或明确 UTC 偏移的 ISO 时间按绝对时刻保存；不带偏移的本地精确时间必须按当前 `settings.timeZone` 解析为唯一绝对时刻；
- `createdAt`：数据进入本系统的审计时间，只记录系统动作，不参与当前预测、修订幅度、事件顺序或事前有效性判断。

表单、Store、JSON 和 CSV 导入共用上述来源时间解析。无偏移时间禁止使用运行环境的裸 `Date.parse()`；例如浏览器处于东京、工作流为上海时，`2026-07-15T15:00` 必须解释为上海 15:00。来源时间遇到 DST gap 或 overlap 时拒绝写入，要求改用带偏移 ISO 或日期精度。`sourcePublishedAtResolution` 记录日期、绝对时刻、工作流时区解析或历史待核验状态；工作流时区解析还记录 `sourcePublishedAtTimeZone`。
...
V1 schema 继续为 `1`。历史快照若保存了无偏移精确来源时间但没有原解释时区，迁移只保留原字符串并标记 `unresolved_legacy`，不会擅自解释成新的绝对时刻，也不会凭该时间证明事前有效；后续人工核验可通过追加纠正快照补齐。
...
事件携带业务根快照 ID、当前纠正链终点 ID、纠正链、来源类别、来源名称、报告期、指标、预期值或区间、原业务时间、纠正记录时间、来源发布时间及其解释状态、业务时间精度、`before/after/equal/uncertain` 关系、披露时间状态、`correctionDelta`、`businessRevisionDelta`、比较结果和来源核验状态。纠正事件明确引用直接目标并使用纠正记录时间；业务修订明确引用上一业务根及其有效终点，二者不混用。只有业务顺序确认且严格可比较的结果生成 `comparison_available`；其他情况生成稳定的数据核验事件。

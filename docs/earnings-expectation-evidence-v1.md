...
公司指引 Provider V2 将稳定证据身份与不可变内容版本拆开：`providerEvidenceIdentity` 表示同一公告业务证据，`providerSnapshotVersionId` 由规范化财务内容 SHA-256 寻址；抽取纠错用 `providerCorrectsVersionId`，公司业务修正公告用独立的 `providerBusinessRevisionPredecessorSnapshotId`。公司内部形成时间未知，记录继续使用 `formationTimeBasis=public_disclosure_proxy`、保留 `formedAt=null`，公开公告时间只作为证据可用时间。详情、全局 workflow index、模式隔离、四类本地关系和验证命令见 [A股公司指引预期 Provider V2](./company-guidance-expectation-provider-v1.md)。

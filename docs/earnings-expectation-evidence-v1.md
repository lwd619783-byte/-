...
V1 schema 仍为 `1`。旧快照迁移时不会用 `createdAt` 推断形成时间；缺少精确时间的旧记录统一按日期精度处理。`snapshots` 缺失或不是数组属于损坏数据，不会迁移为空数组，也不会覆盖原始文本。
...
JSON 导入会执行全部枚举白名单校验，拒绝 `provider` 或伪造值；并根据当前股票主数据把合法代码映射为 stock ID，拒绝孤儿股票和 market 不一致记录。无效记录保留原始问题载荷，不会写入有效快照。
...
事件携带快照 ID、来源类别、来源名称、报告期、指标、预期值或区间、两类披露前状态、事前有效性、比较结果和来源核验状态。比较事件的 `eventDate`/`publishedAt` 来自实际披露或更晚的业务可用时间，不使用每次运行变化的 `calculatedAt`，因此相同业务输入在刷新和重载后生成相同完整事件。用户预测保持 `user_estimate`，不会转换成机构事件。只有严格可比较的结果生成 `comparison_available`；其他情况生成数据核验事件。

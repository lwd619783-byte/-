...
## D5 — cross-theme acceptance and targeted repairs

- Completed the 24 desktop / 8 neon narrow screenshot matrix, 320px, doubled HTML text and 200% equivalent reflow, keyboard navigation / focus / dialogs, reduced motion, and actual A–E workflows. Additional isolated fixtures cover delayed responses, broken preference/storage writes, corrupt storage export, missing/single/gapped price history, long names, large negative values and unavailable Hong Kong financials.
- Fixed measured placeholder/control contrast, narrow touch targets and filter density, enlarged-text chart heading wrap and compact axis formatting. Price selection uses only existing observations and is shared across the company's overview/valuation tabs; financial period/scope remains stable.
- Fixed four migration-review findings: visible positive signs, truthful unknown provenance, both original signal texts, and zero-valued shares. All M01–M56 now have explicit destinations and no unresolved migration findings.
- Windows guidance JSON transport was restored to exact original Git bytes; one `.gitattributes` rule prevents automatic CRLF conversion. All 58 JSON files equal the baseline blobs, with no provider or public-data content diff.
- Final complete test: 54 files / 720 tests PASS. Final build, TypeScript, Local Core browser boundary, original bundle budget, UI static audit, data audit and guidance integrity validator PASS. Data audit retains 24 warnings. Both environment commands report READY WITH WARNINGS, not an unqualified PASS.
- Exact logs, 54-row acceptance mapping, browser methods and limitations, environment SKIP/NOT_RUN and performance evidence are indexed in [acceptance.md](acceptance.md). Final remote SHA / ahead-behind / clean tree and tested-content linkage are in local `final-git.json` / `evidence-manifest.json`.
- D5 uses the requested ordinary commit `fix: complete UI v1 cross-theme acceptance` and ordinary push on the same branch. No PR, main merge, deployment or follow-on scope is performed.

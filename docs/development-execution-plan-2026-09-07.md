...
| Phase 1B — Long-term Account & DCA Core | 保留既有实现 `2968baa` / `d1e343ce`；普通 merge 已审计 main 合同后，执行 CB-1 / CB-2 / CB-3 implementation alignment | 本轮已获实现、验证及普通 push 授权；完成后等待独立远端终局审计，不创建 PR、不合并 main | **AUTHORIZED FOR CONTRACT ALIGNMENT / IN PROGRESS**；原 `d1e343ce` 为 IMPLEMENTED PARTIALLY / PREVIOUS IMPLEMENTATION REVIEW PASS；[历史修复验证](investment-dashboard-v2-phase-1b-review-fixes-validation.md)保留原时点结论 |

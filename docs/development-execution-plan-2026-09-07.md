...
| Phase 1B — Long-term Account & DCA Core | 当前任务已明确授权；复用 Phase 1A Local Core，实现账户、资产、流水、持仓、DCA 和 deterministic import，测试仅用 synthetic/temp DB | 普通 push 后停止，等待独立远端终局审查；不创建 PR / 合并 / 进入下一 Phase；合同缺口须重新审计 | **IMPLEMENTED PARTIALLY / BLOCKED**；[实施映射](investment-dashboard-v2-phase-1b-long-term-account-dca-core.md)与[验证及合同缺口](investment-dashboard-v2-phase-1b-implementation-validation.md)；A-001 / A-002 / A-005 部分场景尚缺冻结表达，未宣称独立审查或 CI 通过 |
